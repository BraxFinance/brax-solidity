...
contract('FraxGaugeController-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		console.log(
			chalk.hex('#ff8b3d').bold('========== Set up the V1 gauge controller with 2 fake farms =========='),
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('========== Set up the V2 gauge controller with 2 fake farms =========='),
		);
		...
		console.log(
			chalk.hex('#f3f3f3').bold(`------ [2] Votes all for Farm 1 only [${FAKE_FARM_1_ADDR.slice(0, 8)}] ------`),
		);
		...
		console.log(
			chalk.hex('#f3f3f3').bold(`------ [3] Votes all for Farm 2 only [${FAKE_FARM_2_ADDR.slice(0, 8)}] ------`),
		);
		...
		for (let j = 0; j < n_gauges_v1; j++) {
			...
			// Sync the weights
			await frax_gauge_controller.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges, {
				from: accounts[9],
			});
		...
		for (let j = 0; j < n_gauges_v2; j++) {
			...
			// Sync the weights
			await frax_gauge_controller_V2.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges, {
				from: accounts[9],
			});
			console.log(`${gauge_name} [${gauge_addr}] checkpointed`);
		}
		...
		for (let j = 0; j < n_gauges_v1; j++) {
			...
			let gauge_rel_wgt = new BigNumber(await frax_gauge_controller.gauge_relative_weight(gauge_addr))
				.div(BIG18)
				.toNumber();
			gauge_rel_wgt *= 100;
		...
		for (let j = 0; j < n_gauges_v2; j++) {
			...
			// Print out the gauge relative weight
			let gauge_rel_wgt = new BigNumber(await frax_gauge_controller_V2.gauge_relative_weight(gauge_addr))
				.div(BIG18)
				.toNumber();
			gauge_rel_wgt *= 100;
		...
		for (let j = 0; j < n_gauges_v1; j++) {
			...
			// Sync the weights
			await frax_gauge_controller.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges_2, {
				from: accounts[9],
			});
			console.log(`${gauge_name} [${gauge_addr}] checkpointed`);
		}
		...
		for (let j = 0; j < n_gauges_v2; j++) {
			...
			// Sync the weights
			await frax_gauge_controller_V2.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges_2, {
				from: accounts[9],
			});
			console.log(`${gauge_name} [${gauge_addr}] checkpointed`);
		}
		...
		for (let j = 0; j < n_gauges_v1; j++) {
			...
			// Sync the weights
			await frax_gauge_controller.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges_2a, {
				from: accounts[9],
			});
			console.log(`${gauge_name} [${gauge_addr}] checkpointed`);
		}
		...
		for (let j = 0; j < n_gauges_v2; j++) {
			...
			// Sync the weights
			await frax_gauge_controller_V2.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges_2a, {
				from: accounts[9],
			});
			console.log(`${gauge_name} [${gauge_addr}] checkpointed`);
		}
		...
		console.log(
			chalk.hex('#ff8b3d').bold('================== Checkpoint the V1 gauges a third time =================='),
		);
		...
		for (let j = 0; j < n_gauges_v1; j++) {
			...
			// Sync the weights
			await frax_gauge_controller.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges_3, {
				from: accounts[9],
			});
			console.log(`${gauge_name} [${gauge_addr}] checkpointed`);
		}
		...
		console.log(
			chalk.hex('#ff8b3d').bold('================== Checkpoint the V2 gauges a third time =================='),
		);
		for (let j = 0; j < n_gauges_v2; j++) {
			// Get the address and name
			const gauge_addr = await frax_gauge_controller_V2.gauges(j);
			...
			// Sync the weights
			await frax_gauge_controller_V2.gauge_relative_weight_write(gauge_addr, current_timestamp_for_gauges_3, {
				from: accounts[9],
			});
			console.log(`${gauge_name} [${gauge_addr}] checkpointed`);
		}
		...
		console.log(
			chalk.hex('#ff8b3d').bold('================== Print the V1 weights a third time =================='),
		);
		// Must be done after checkpoint the gauges
		let v1_weights_3 = {};
		for (let j = 0; j < n_gauges_v1; j++) {
		...
		console.log(
			chalk.hex('#ff8b3d').bold('================== Print the V2 weights a third time =================='),
		);
		// Must be done after checkpoint the gauges
		let v2_weights_3 = {};
		for (let j = 0; j < n_gauges_v2; j++) {
