...
contract('ComboOracle_UniV2_UniV3-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=====================GET CONSTITUENT TOKEN PRICES====================='),
		);
		...
		for (let i = 0; i < univ2_test_token_addresses.length; i++) {
			let token_price_pack = await combo_oracle_instance_univ2_univ3.uniV2LPPriceInfoViaReserves.call(
				univ2_test_token_addresses[i],
			);
		...
		for (let i = 0; i < univ2_test_token_addresses.length; i++) {
			let token_price_pack = await combo_oracle_instance_univ2_univ3.uniV2LPPriceInfo.call(
				univ2_test_token_addresses[i],
			);
			let token_price = new BigNumber(token_price_pack[0]).div(BIG18).toString();
			let token_symbol = token_price_pack[2];
		...
		for (let i = 0; i < univ3_test_nft_ids.length; i++) {
			...
			console.log(
				`UniV3 #${the_token_id} ${token_0_symbol}/${token_1_symbol} Total Value: $${token_ttl_value} [${token_0_value}/${token_1_value}] {Liq price: ${liquidity_price}}`,
			);
		}
	});
});
