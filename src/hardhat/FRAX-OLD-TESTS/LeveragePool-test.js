...
describe('LeveragePool', function () {
	...
	it('setFees', async function () {
		var leveragePool = await setupLeveragePool();
		await expect(leveragePool.setFees('1000000000000000', '400000000000000000', '700000000000000000')).to.be
			.reverted; // 0.1%,40%,70% --> Fees do not sum
		await expect(leveragePool.setFees('-1000000000000000', '300000000000000000', '700000000000000000')).to.be
			.reverted; // -0.1%,30%,70% --> No negatives
		await expect(leveragePool.setFees('1000000000000000', '-300000000000000000', '700000000000000000')).to.be
			.reverted; // 0.1%,-30%,70% --> No negatives
		await expect(leveragePool.setFees('1000000000000000', '300000000000000000', '-700000000000000000')).to.be
			.reverted; // 0.1%,30%,-70% --> No negatives
		await leveragePool.setFees('20000000000000000', '300000000000000000', '700000000000000000'); // 2%,30%,70%
		await expect(leveragePool.setFees('20000000000000001', '300000000000000000', '700000000000000000')).to.be
			.reverted; // 2.0000000000000001%,30%,70% --> Fee to high
...
function checkRates(
	...
	longFundingRate,
	shortFundingRate,
	liquidityPoolFundingRate,
	rebalanceRate,
	rebalanceLiquidityPoolRate,
) {
