...
contract('veFXS Tests', async (accounts) => {
	...
	it('Check up on the oracles and make sure the prices are set', async () => {
		...
		await fxs_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('1000000e18'), {
			from: ADDRESS_WITH_FXS,
		});
		await fxs_instance.transfer(INVESTOR_CUSTODIAN_ADDRESS, new BigNumber('1000000e18'), {
			from: ADDRESS_WITH_FXS,
		});
		...
		console.log(
			'accounts[1] realUSDC balance:',
			new BigNumber(await usdc_real_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG6).toNumber(),
		);
	...
	it('Tests veFXS', async () => {
		...
		await veFXS_instance.create_lock(deposit_amount_quick_e18_4_yr, deposit_quick_timestamp_4_yr, {
			from: STAKING_OWNER,
		});
		...
		for (let j = 0; j <= LOOP_MAX_4_YR; j++) {
			...
			const veFXS_balance_mid = new BigNumber(await veFXS_instance.balanceOf(STAKING_OWNER))
				.div(BIG18)
				.toNumber();
		...
		const deposit_quick_timestamp_30_days_0 =
			block_time_current_30_days_0 + (deposit_quick_days_30_days * 86400 + 1);
		await fxs_instance.approve(veFXS_instance.address, deposit_amount_quick_e18_30_days, { from: STAKING_OWNER });
		await veFXS_instance.create_lock(deposit_amount_quick_e18_30_days, deposit_quick_timestamp_30_days_0, {
			from: STAKING_OWNER,
		});
		...
		console.log(
			`Advance ${LOOP_MAX_30_DAYS} blocks and ${deposit_quick_days_30_days} days, checkpointing each time`,
		);
		...
		const deposit_quick_timestamp_30_days_1 =
			block_time_current_30_days_1 + (deposit_quick_days_30_days * 86400 + 1);
		await fxs_instance.approve(veFXS_instance.address, deposit_amount_quick_e18_30_days, { from: STAKING_OWNER });
		await veFXS_instance.create_lock(deposit_amount_quick_e18_30_days, deposit_quick_timestamp_30_days_1, {
			from: STAKING_OWNER,
		});
		...
		const pre_deposit_veFXS = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		...
		const post_deposit_veFXS = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		...
		await fxs_instance.approve(veFXS_instance.address, deposit_amount_2_e18, {
		...
		const veFXS_after_adding_more = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		...
		const veFXS_before_lock_extension = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		...
		await veFXS_instance.increase_unlock_time(lock_extension_new_timestamp, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		const veFXS_after_lock_extension = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		...
		const pre_withdrawal_FXS_2 = new BigNumber(await fxs_instance.balanceOf(INVESTOR_CUSTODIAN_ADDRESS))
			.div(BIG18)
			.toNumber();
		const pre_withdrawal_veFXS_2 = new BigNumber(await veFXS_instance.balanceOf(INVESTOR_CUSTODIAN_ADDRESS))
			.div(BIG18)
			.toNumber();
		...
		const post_withdrawal_FXS_2 = new BigNumber(await fxs_instance.balanceOf(INVESTOR_CUSTODIAN_ADDRESS))
			.div(BIG18)
			.toNumber();
		const post_withdrawal_veFXS_2 = new BigNumber(await veFXS_instance.balanceOf(INVESTOR_CUSTODIAN_ADDRESS))
			.div(BIG18)
			.toNumber();

		console.log(
			`post_withdrawal_FXS_2: ${post_withdrawal_FXS_2} (change: ${post_withdrawal_FXS_2 - pre_withdrawal_FXS_2})`,
		);
		console.log(
			`post_withdrawal_veFXS_2: ${post_withdrawal_veFXS_2} (change: ${
				post_withdrawal_veFXS_2 - pre_withdrawal_veFXS_2
			})`,
		);
		...
		const veFXS_total_supply_10_blocks_before = new BigNumber(await veFXS_instance.totalSupplyAt(block_10_ago))
			.div(BIG18)
			.toNumber();
		const veFXS_account1_balance = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const veFXS_account1_balance_10_blocks_before = new BigNumber(
			await veFXS_instance.balanceOfAt(COLLATERAL_FRAX_AND_FXS_OWNER, block_10_ago),
		)
			.div(BIG18)
			.toNumber();
		...
		console.log(
			"veFXS 'balance' should decrease as it gets closer to the expiry. However, the FXS will not be lost",
		);
		console.log('COLLATERAL_FRAX_AND_FXS_OWNER veFXS_account1_balance:', veFXS_account1_balance);
		console.log(
			'COLLATERAL_FRAX_AND_FXS_OWNER veFXS_balance_10_blocks_before:',
			veFXS_account1_balance_10_blocks_before,
		);
		...
		const veFXS_total_supply_10_blocks_before_1 = new BigNumber(await veFXS_instance.totalSupplyAt(block_10_ago_1))
			.div(BIG18)
			.toNumber();
		const veFXS_account1_balance_1 = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const veFXS_account1_balance_10_blocks_before_1 = new BigNumber(
			await veFXS_instance.balanceOfAt(COLLATERAL_FRAX_AND_FXS_OWNER, block_10_ago_1),
		)
			.div(BIG18)
			.toNumber();
		...
		console.log(
			'COLLATERAL_FRAX_AND_FXS_OWNER veFXS_balance_10_blocks_before:',
			veFXS_account1_balance_10_blocks_before_1,
		);
		...
		console.log('Get FXS and veFXS balances before');
		const pre_withdrawal_FXS = new BigNumber(await fxs_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const pre_withdrawal_veFXS = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		...
		console.log('Get FXS and veFXS balances after');
		const post_withdrawal_FXS = new BigNumber(await fxs_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const post_withdrawal_veFXS = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		console.log(
			`post_withdrawal_FXS: ${post_withdrawal_FXS} (change: ${post_withdrawal_FXS - pre_withdrawal_FXS})`,
		);
		console.log(
			`post_withdrawal_veFXS: ${post_withdrawal_veFXS} (change: ${post_withdrawal_veFXS - pre_withdrawal_veFXS})`,
		);
		...
		const veFXS_total_supply_10_blocks_before_2 = new BigNumber(await veFXS_instance.totalSupplyAt(block_10_ago_2))
			.div(BIG18)
			.toNumber();
		const veFXS_account1_balance_2 = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const veFXS_account1_balance_10_blocks_before_2 = new BigNumber(
			await veFXS_instance.balanceOfAt(COLLATERAL_FRAX_AND_FXS_OWNER, block_10_ago_2),
		)
			.div(BIG18)
			.toNumber();
		...
		console.log(
			'COLLATERAL_FRAX_AND_FXS_OWNER veFXS_balance_10_blocks_before:',
			veFXS_account1_balance_10_blocks_before_2,
		);
		...
		const pre_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v3_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit Curve AMO 3pool LP balance:', pre_deposit_3pool_usdc_only);
		console.log(
			'pre-deposit Curve AMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(curve_amo_v3_instance.address)).div(BIG6).toNumber(),
		);
		console.log('');

		console.log('Depositing 10k USDC into the metapool');
		await curve_amo_v3_instance.metapoolDeposit(0, new BigNumber('10000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('');
		const post_deposit_metapool_usdc_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_amo_v3_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('post-deposit Curve AMO metapool LP balance:', post_deposit_metapool_usdc_only);
		const post_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v3_instance.address))
			.div(BIG18)
			.toNumber();
		...
		console.log('');
		console.log(
			'Curve AMO collatDollarBalance():',
			new BigNumber(await curve_amo_v3_instance.collatDollarBalance()).div(BIG18).toNumber(),
		);
		...
		const pre_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v3_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await curve_amo_v3_instance.metapoolDeposit(new BigNumber('100000e18'), 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v3_instance.address))
			.div(BIG18)
			.toNumber();
		...
		const pre_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(curve_amo_v3_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await curve_amo_v3_instance.metapoolDeposit(new BigNumber('5000e18'), new BigNumber('5000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(curve_amo_v3_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await curve_amo_v3_instance.metapoolWithdrawAndConvert3pool(test_amt_3pool_both, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_amo_v3_instance.metapoolWithdrawAtCurRatio(test_amt_frax_usdc, 0, 0, 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_amo_v3_instance.withdrawFromGauge(new BigNumber('2500e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const fxs_balance_after = new BigNumber(await fxs_instance.balanceOf.call(curve_amo_v3_instance.address)).div(
			BIG18,
		);
		const usdc_balance_after = new BigNumber(await usdc_instance.balanceOf.call(curve_amo_v3_instance.address)).div(
			BIG6,
		);
		...
		console.log(
			'CurveAMO_V4 collatDollarBalance():',
			new BigNumber(await curve_amo_v3_instance.collatDollarBalance()).div(BIG18).toNumber(),
		);
