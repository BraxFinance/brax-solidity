...
contract('FraxMiddlemanGauge-Tests', async (accounts) => {
	...
	it('Initialization related', async () => {
		...
		await fxs_instance.transfer(gauge_rewards_distributor_instance.address, new BigNumber('500000e18'), {
			from: ADDRESS_WITH_FXS,
		});
		...
		await frax_gauge_controller_v2.vote_for_gauge_weights(middleman_instance.address, 9000, {
		...
		await gauge_rewards_distributor_instance.setGaugeState(middleman_instance.address, 1, 1, {
	...
	it('Test FXS reward amounts', async () => {
		...
		const current_timestamp_0 = new BigNumber(await time.latest()).toNumber();
		await frax_gauge_controller_v2.gauge_relative_weight_write(fraxFarmInstance_FRAX_DAI.address, {
			from: accounts[9],
		});
		await frax_gauge_controller_v2.gauge_relative_weight_write(fraxFarmInstance_FRAX_USDC.address, {
			from: accounts[9],
		});
		...
		const estimated_rewards_0_frax_usdc = new BigNumber(
			...
			.div(BIG18)
			.toNumber();
		const estimated_rewards_0_middleman = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(middleman_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		const estimated_rewards_1_frax_usdc = new BigNumber(
			...
			.div(BIG18)
			.toNumber();
		const estimated_rewards_1_middleman = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(middleman_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		const estimated_rewards_2_frax_usdc = new BigNumber(
			...
			.div(BIG18)
			.toNumber();
		const estimated_rewards_2_middleman = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(middleman_instance.address),
		)
			.div(BIG18)
			.toNumber();
