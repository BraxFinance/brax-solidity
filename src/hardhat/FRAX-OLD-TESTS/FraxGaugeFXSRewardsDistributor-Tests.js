...
contract('FraxGaugeFXSRewardsDistributor-Tests', async (accounts) => {
	...
	it('Initialization related', async () => {
		...
		await fxs_instance.transfer(gauge_rewards_distributor_instance.address, new BigNumber('1000000e18'), {
	...
	it('Test FXS reward amounts', async () => {
		...
		const fxs_bal_distributor_before_reward_0 = new BigNumber(
			await fxs_instance.balanceOf.call(gauge_rewards_distributor_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		await frax_gauge_controller.gauge_relative_weight_write(middlemanGauge_FRAX_mUSD.address, current_timestamp_0, {
			from: accounts[9],
		});

		// Get estimated rewards
		const estimated_rewards_0_frax_dai = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(fraxFarmInstance_FRAX_DAI.address),
		)
			.div(BIG18)
			.toNumber();
		const estimated_rewards_0_frax_usdc = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(fraxFarmInstance_FRAX_USDC.address),
		)
			.div(BIG18)
			.toNumber();
		const estimated_rewards_0_musd_frax = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(middlemanGauge_FRAX_mUSD.address),
		)
			.div(BIG18)
			.toNumber();
		...
		await gauge_rewards_distributor_instance.distributeReward(fraxFarmInstance_FRAX_DAI.address, {
			from: accounts[9],
		});
		await gauge_rewards_distributor_instance.distributeReward(fraxFarmInstance_FRAX_USDC.address, {
			from: accounts[9],
		});
		await gauge_rewards_distributor_instance.distributeReward(middlemanGauge_FRAX_mUSD.address, {
			from: accounts[9],
		});

		// Note balances afterwards. Should have FRAX and USDC LP fees too
		const fxs_bal_distributor_after_reward_0 = new BigNumber(
			await fxs_instance.balanceOf.call(gauge_rewards_distributor_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		// Print the changes
		console.log(
			`FXS change from reward: ${fxs_bal_distributor_after_reward_0 - fxs_bal_distributor_before_reward_0} FXS`,
		);
		assert(
			fxs_bal_distributor_after_reward_0 < fxs_bal_distributor_before_reward_0,
		...
		// Note balances beforehand
		const fxs_bal_distributor_before_reward_1 = new BigNumber(
			await fxs_instance.balanceOf.call(gauge_rewards_distributor_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		await frax_gauge_controller.gauge_relative_weight_write(middlemanGauge_FRAX_mUSD.address, current_timestamp_1, {
			from: accounts[9],
		});

		// Get estimated rewards
		const estimated_rewards_1_frax_dai = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(fraxFarmInstance_FRAX_DAI.address),
		)
			.div(BIG18)
			.toNumber();
		const estimated_rewards_1_frax_usdc = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(fraxFarmInstance_FRAX_USDC.address),
		)
			.div(BIG18)
			.toNumber();
		const estimated_rewards_1_musd_frax = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(middlemanGauge_FRAX_mUSD.address),
		)
			.div(BIG18)
			.toNumber();
		...
		// Note balances afterwards. Should have FRAX and USDC LP fees too
		const fxs_bal_distributor_after_reward_1 = new BigNumber(
			await fxs_instance.balanceOf.call(gauge_rewards_distributor_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		// Print the changes
		console.log(
			`FXS change from reward: ${fxs_bal_distributor_after_reward_1 - fxs_bal_distributor_before_reward_1} FXS`,
		);
		assert(
			fxs_bal_distributor_after_reward_1 == fxs_bal_distributor_before_reward_1,
		...
		// Note balances beforehand
		const fxs_bal_distributor_before_reward_2 = new BigNumber(
			await fxs_instance.balanceOf.call(gauge_rewards_distributor_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		await frax_gauge_controller.gauge_relative_weight_write(middlemanGauge_FRAX_mUSD.address, current_timestamp_2, {
			from: accounts[9],
		});

		// Get estimated rewards
		const estimated_rewards_2_frax_dai = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(fraxFarmInstance_FRAX_DAI.address),
		)
			.div(BIG18)
			.toNumber();
		const estimated_rewards_2_frax_usdc = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(fraxFarmInstance_FRAX_USDC.address),
		)
			.div(BIG18)
			.toNumber();
		const estimated_rewards_2_musd_frax = new BigNumber(
			await gauge_rewards_distributor_instance.currentReward.call(middlemanGauge_FRAX_mUSD.address),
		)
			.div(BIG18)
			.toNumber();
		...
		// Note balances afterwards. Should have FRAX and USDC LP fees too
		const fxs_bal_distributor_after_reward_2 = new BigNumber(
			await fxs_instance.balanceOf.call(gauge_rewards_distributor_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		// Print the changes
		console.log(
			`FXS change from reward: ${fxs_bal_distributor_after_reward_2 - fxs_bal_distributor_before_reward_2} FXS`,
		);
		assert(
			fxs_bal_distributor_after_reward_2 < fxs_bal_distributor_before_reward_2,
			'Should have emitted something',
		);
		...
		await expectRevert(
			gauge_rewards_distributor_instance.distributeReward(fraxFarmInstance_FRAX_DAI.address, {
				from: accounts[9],
			}),
		...
		await expectRevert(
			gauge_rewards_distributor_instance.distributeReward(fraxFarmInstance_FRAX_USDC.address, {
				from: accounts[9],
			}),
			'Distributions are off',
		);
		...
		await expectRevert(
			gauge_rewards_distributor_instance.distributeReward(middlemanGauge_FRAX_mUSD.address, {
				from: accounts[9],
			}),
			'Distributions are off',
		);
