...
contract('SushiSwapLiquidityAMO_POLY-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		// Total profits
		let borrowed_frax = new BigNumber(await SushiSwapLiquidityAMO_POLY_instance.borrowed_frax.call())
			.div(BIG18)
			.toNumber();
		let borrowed_fxs = new BigNumber(await SushiSwapLiquidityAMO_POLY_instance.borrowed_fxs.call())
			.div(BIG18)
			.toNumber();
		let borrowed_collat = new BigNumber(await SushiSwapLiquidityAMO_POLY_instance.borrowed_collat.call())
			.div(BIG6)
			.toNumber();
		let total_profit = new BigNumber(await SushiSwapLiquidityAMO_POLY_instance.total_profit.call())
			.div(BIG18)
			.toNumber();
