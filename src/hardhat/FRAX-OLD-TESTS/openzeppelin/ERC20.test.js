...
const { PERMIT_TYPEHASH, getPermitDigest, getDomainSeparator, sign } = require(path.join(
	__dirname,
	'../../../../dist/hardhat/test/openzeppelin/signatures',
));
...
contract('CrossChainCanonical', function (accounts) {
	...
	it('initializes DOMAIN_SEPARATOR and PERMIT_TYPEHASH correctly', async () => {
		...
		assert.equal(
			await token_for_permit.DOMAIN_SEPARATOR(),
			getDomainSeparator(name, token_for_permit.address, chainId),
		);
	});
