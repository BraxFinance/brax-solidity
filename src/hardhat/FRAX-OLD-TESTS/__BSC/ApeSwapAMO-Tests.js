...
contract('ApeSwapAMO-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await usdc_instance.transfer(pangolinLiquidityAMO_instance.address, new BigNumber('100000e6'), {
		...
		console.log('Give the bridge backer some canFRAX and canFXS');
		await canFRAX_instance.transfer(pangolinLiquidityAMO_instance.address, initial_dump_amt_amo_canToken, {
			from: GOVERNOR_GUARDIAN_ADDRESS,
		});
		await canFXS_instance.transfer(pangolinLiquidityAMO_instance.address, initial_dump_amt_amo_canToken, {
			from: GOVERNOR_GUARDIAN_ADDRESS,
		});
		...
		const FXS_PRICE_FROM_LP0_E18 = new BigNumber(
			await pangolinLiquidityAMO_instance.pair_reserve_ratio_E18.call(
		...
		await pangolinLiquidityAMO_instance.giveCollatBack(new BigNumber('250e6'), false, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await pangolinLiquidityAMO_instance.giveFRAXBack(new BigNumber('1e18'), true, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Give back 1 canFXS');
		await pangolinLiquidityAMO_instance.giveFXSBack(new BigNumber('1e18'), true, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await pangolinLiquidityAMO_instance.execute(canFRAX_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
