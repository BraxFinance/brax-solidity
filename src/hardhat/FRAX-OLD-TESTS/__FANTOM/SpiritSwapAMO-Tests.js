...
contract('SpiritSwapLiquidityAMO-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await anyUSDC_instance.transfer(cc_bridge_backer_instance.address, new BigNumber('100000e6'), {
		...
		const FXS_ORACLE_PRICE_E6 = new BigNumber(
			await cross_chain_oracle_instance.getPrice.call(CONTRACT_ADDRESSES.fantom.canonicals.FXS),
		);
		...
		console.log(
			'Total Profit:',
			new BigNumber(await spiritSwapLiquidityAMO_instance.total_profit.call()).div(BIG18).toNumber(),
		);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold(
					'===========================LEND SOME CANONICAL TOKENS TO THE SpiritSwapLiquidityAMO===========================',
				),
		);
		...
		await cc_bridge_backer_instance.lendFxsToAMO(spiritSwapLiquidityAMO_instance.address, new BigNumber('250e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const FXS_PRICE_FROM_LP0_E18 = new BigNumber(
			await spiritSwapLiquidityAMO_instance.pair_reserve_ratio_E18.call(
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('===========================CREATE SOME FRAX/anyUSDC LP TOKENS==========================='),
		);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('===========================CREATE SOME FXS/anyUSDC LP TOKENS==========================='),
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=======SWAP SOME FRAX FOR FXS VIA AN EOA, TO EARN FEES FOR THE AMO======='),
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=======SWAP SOME FRAX FOR USDC VIA AN EOA, TO EARN FEES FOR THE AMO======='),
		);
		const eoa_swap_amt_back = new BigNumber('10e18');

		await canFRAX_instance.approve(swap_router_instance.address, eoa_swap_amt_back, {
			from: GOVERNOR_GUARDIAN_ADDRESS,
		});
		...
		await spiritSwapLiquidityAMO_instance.giveFRAXBack(new BigNumber('25e18'), false, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Give back 25 canFXS');
		await spiritSwapLiquidityAMO_instance.giveFXSBack(new BigNumber('25e18'), false, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Give back 25 USDC');
		await spiritSwapLiquidityAMO_instance.giveCollatBack(new BigNumber('25e6'), false, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await spiritSwapLiquidityAMO_instance.giveFRAXBack(new BigNumber('1e18'), true, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Give back 1 canFXS');
		await spiritSwapLiquidityAMO_instance.giveFXSBack(new BigNumber('1e18'), true, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await spiritSwapLiquidityAMO_instance.execute(canFRAX_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
