...
contract('FPI-FPIS-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await fpi_instance.transfer(INVESTOR_CUSTODIAN_ADDRESS, transfer_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await fpis_instance.transfer(INVESTOR_CUSTODIAN_ADDRESS, transfer_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Approve for transferFrom');
		await fpi_instance.approve(INVESTOR_CUSTODIAN_ADDRESS, transfer_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await fpis_instance.approve(INVESTOR_CUSTODIAN_ADDRESS, transfer_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Transfer a test amount');
		await fpi_instance.transferFrom(
			COLLATERAL_FRAX_AND_FXS_OWNER,
			INVESTOR_CUSTODIAN_ADDRESS,
			transfer_test_amount,
			{ from: INVESTOR_CUSTODIAN_ADDRESS },
		);
		await fpis_instance.transferFrom(
			COLLATERAL_FRAX_AND_FXS_OWNER,
			INVESTOR_CUSTODIAN_ADDRESS,
			transfer_test_amount,
			{ from: INVESTOR_CUSTODIAN_ADDRESS },
		...
		await fpi_instance.minter_mint(COLLATERAL_FRAX_AND_FXS_OWNER, mint_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await fpis_instance.minter_mint(COLLATERAL_FRAX_AND_FXS_OWNER, mint_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await fpi_instance.approve(COLLATERAL_FRAX_AND_FXS_OWNER, burn_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await fpis_instance.approve(COLLATERAL_FRAX_AND_FXS_OWNER, burn_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('BurnFrom a test amount');
		await fpi_instance.minter_burn_from(COLLATERAL_FRAX_AND_FXS_OWNER, burn_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await fpis_instance.minter_burn_from(COLLATERAL_FRAX_AND_FXS_OWNER, burn_test_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await expectRevert(
			fpi_instance.minter_mint(INVESTOR_CUSTODIAN_ADDRESS, mint_test_amount, {
				from: INVESTOR_CUSTODIAN_ADDRESS,
			}),
			'Only minters',
		);
		await expectRevert(
			fpis_instance.minter_mint(INVESTOR_CUSTODIAN_ADDRESS, mint_test_amount, {
				from: INVESTOR_CUSTODIAN_ADDRESS,
			}),
			'Only minters',
		);
		...
		await expectRevert(
			fpi_instance.transferFrom(COLLATERAL_FRAX_AND_FXS_OWNER, INVESTOR_CUSTODIAN_ADDRESS, transfer_test_amount, {
				from: INVESTOR_CUSTODIAN_ADDRESS,
			}),
			'ERC20: insufficient allowance',
		);
		await expectRevert(
			fpis_instance.transferFrom(
				COLLATERAL_FRAX_AND_FXS_OWNER,
				INVESTOR_CUSTODIAN_ADDRESS,
				transfer_test_amount,
				{ from: INVESTOR_CUSTODIAN_ADDRESS },
