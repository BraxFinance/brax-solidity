...
// Staking contracts
const FraxUnifiedFarm_ERC20_Gelato_FRAX_DAI = artifacts.require(
	'Staking/Variants/FraxUnifiedFarm_ERC20_Gelato_FRAX_DAI',
);
const FraxUnifiedFarm_ERC20_Temple_FRAX_TEMPLE = artifacts.require(
	'Staking/Variants/FraxUnifiedFarm_ERC20_Temple_FRAX_TEMPLE',
);
const FraxUnifiedFarm_ERC20_Vesper_Orbit_FRAX = artifacts.require(
	'Staking/Variants/FraxUnifiedFarm_ERC20_Vesper_Orbit_FRAX',
);
...
contract('FraxUnifiedFarm_ERC20-Tests', async (accounts) => {
	...
	beforeEach(async () => {
		...
		pair_instance_Temple_FRAX_TEMPLE = await IUniswapV2Pair.at(
	...
	it('Main test', async () => {
		...
		await fxs_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('1000e18'), {
			from: ADDRESS_WITH_FXS,
		});
		...
		await temple_frax_amm_ops.withdraw(pair_instance.address, accounts[1], new BigNumber('30e18'), {
			from: ADDRESS_LP_ADMIN,
		});
		await temple_frax_amm_ops.withdraw(pair_instance.address, accounts[9], new BigNumber('30e18'), {
			from: ADDRESS_LP_ADMIN,
		});
		...
		await frax_gauge_controller.vote_for_gauge_weights(staking_instance.address, 10000, {
		...
		await gauge_rewards_distributor_instance.setGaugeState(staking_instance.address, 0, 1, {
		...
		await frax_gauge_controller.gauge_relative_weight_write('0x3EF26504dbc8Dd7B7aa3E97Bc9f3813a9FC0B4B0', {
			from: accounts[9],
		});
		await frax_gauge_controller.gauge_relative_weight_write('0xF22471AC2156B489CC4a59092c56713F813ff53e', {
			from: accounts[9],
		});
		await frax_gauge_controller.gauge_relative_weight_write('0x3e14f6EEDCC5Bc1d0Fc7B20B45eAE7B1F74a6AeC', {
			from: accounts[9],
		});
		...
		const gauge_rel_wgt = new BigNumber(
			await frax_gauge_controller.gauge_relative_weight.call(staking_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		const weekly_total_emission = new BigNumber(await frax_gauge_controller.global_emission_rate.call())
			.multipliedBy(604800)
			.div(BIG18)
			.toNumber();
		...
		let reward_amount = new BigNumber(await staking_instance.rewardRates.call(0))
			.multipliedBy(604800)
			.div(BIG18)
			.toNumber();
		...
		console.log(
			chalk.yellow.bold(
				'DEPENDING ON WHO VOTED, THE REWARD RATE MAY CHANGE SLIGHTLY WEEK AFTER WEEK DUE TO VEFXS DECAY',
			),
		);
	});
	...
	it('Locked stakes', async () => {
		...
		// Print FXS balances
		const fxs_bal_week_0 = new BigNumber(await fxs_instance.balanceOf(staking_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('FXS balance week 0:', fxs_bal_week_0);
		console.log(
			'accounts[1] FXS balance:',
			new BigNumber(await fxs_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18).toNumber(),
		);
		console.log(
			'accounts[9] FXS balance:',
			new BigNumber(await fxs_instance.balanceOf(accounts[9])).div(BIG18).toNumber(),
		);
		...
		const vefxs_bst_scl_fctr = new BigNumber(await staking_instance.vefxs_boost_scale_factor.call())
			.div(BIG18)
			.toNumber();
		const vefxs_max_mult = new BigNumber(await staking_instance.vefxs_max_multiplier.call()).div(BIG18).toNumber();
		const vefxs_per_fx_max_bst = new BigNumber(await staking_instance.vefxs_per_frax_for_max_boost.call())
			.div(BIG18)
			.toNumber();
		...
		const vefxs_bal_1_bef = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const vefxs_mult_1_bef = new BigNumber(await staking_instance.veFXSMultiplier(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const min_vefxs_max_boost_1_bef = new BigNumber(
			await staking_instance.minVeFXSForMaxBoost(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		const user_staked_frax_1_bef = new BigNumber(
			await staking_instance.userStakedFrax(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		...
		await pair_instance.approve(staking_instance.address, liq_locked_1_sum, {
		...
		const vefxs_bal_1_aft = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const vefxs_mult_1_aft = new BigNumber(await staking_instance.veFXSMultiplier(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const min_vefxs_max_boost_1_aft_bn = new BigNumber(
			await staking_instance.minVeFXSForMaxBoost(COLLATERAL_FRAX_AND_FXS_OWNER),
		);
		const min_vefxs_max_boost_1_aft = min_vefxs_max_boost_1_aft_bn.div(BIG18).toNumber();
		const user_staked_frax_1_aft = new BigNumber(
			await staking_instance.userStakedFrax(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		...
		assert(
			min_vefxs_max_boost_1_aft / user_staked_frax_1_aft >= vefxs_per_fx_max_bst * 0.99,
			'minVeFXSForMaxBoost / userStakedFrax error [too low]',
		);
		assert(
			min_vefxs_max_boost_1_aft / user_staked_frax_1_aft <= vefxs_per_fx_max_bst * 1.01,
			'minVeFXSForMaxBoost / userStakedFrax error [too high]',
		);
		...
		const fxs_needed_at_4_year_for_1x_bst = fxs_needed_at_4_year_for_max_bst
			.div(2)
		...
		await fxs_instance.approve(veFXS_instance.address, fxs_needed_at_4_year_for_1x_bst, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await veFXS_instance.create_lock(fxs_needed_at_4_year_for_1x_bst, veFXS_deposit_end_timestamp_1, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log(chalk.hex('#f3f3f3').bold('------Note after veFXS increase------'));
		const vefxs_bal_1_aft_vefxs = new BigNumber(await veFXS_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const vefxs_mult_1_aft_vefxs = new BigNumber(
			await staking_instance.veFXSMultiplier(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		const min_vefxs_max_boost_1_aft_vefxs = new BigNumber(
			await staking_instance.minVeFXSForMaxBoost(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		const user_staked_frax_1_aft_vefxs = new BigNumber(
			await staking_instance.userStakedFrax(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		...
		assert(vefxs_mult_1_aft_vefxs >= 0.99, 'veFXSMultiplier error [too low]');
		...
		// Stake Locked
		// account[1]
		await staking_instance.stakeLocked(new BigNumber('25e17'), 365 * 86400, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		}); // 270 days
		...
		const regular_balance_1 = new BigNumber(
			await staking_instance.lockedLiquidityOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const boosted_balance_1 = new BigNumber(
			await staking_instance.combinedWeightOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		...
		await expectRevert.unspecified(
			staking_instance.withdrawLocked(locked_stake_structs_1_0[0].kek_id, COLLATERAL_FRAX_AND_FXS_OWNER, {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			}),
		);
		await expectRevert.unspecified(
			staking_instance.withdrawLocked(locked_stake_structs_9_0[0].kek_id, accounts[9], { from: accounts[9] }),
		);
		await time.advanceBlock();
		...
		// Print the reward rate
		let reward_amount_check1 = new BigNumber(await staking_instance.rewardRates.call(0))
			.multipliedBy(604800)
			.div(BIG18)
			.toNumber();
		...
		// Print out the gauge relative weights
		const gauge_rel_wgt_check_0 = new BigNumber(
			await frax_gauge_controller.gauge_relative_weight.call(staking_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		// Print the weekly emission
		const weekly_total_emission_check_0 = new BigNumber(await frax_gauge_controller.global_emission_rate.call())
			.multipliedBy(604800)
			.div(BIG18)
			.toNumber();
		...
		// Note the UNI POOL and FXS amount after staking
		const regular_balance_00_1 = new BigNumber(
			await staking_instance.lockedLiquidityOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const boosted_balance_00_1 = new BigNumber(
			await staking_instance.combinedWeightOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const locked_balance_00_1 = new BigNumber(
			await staking_instance.lockedLiquidityOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const regular_balance_00_9 = new BigNumber(await staking_instance.lockedLiquidityOf.call(accounts[9])).div(
			BIG18,
		);
		const boosted_balance_00_9 = new BigNumber(await staking_instance.combinedWeightOf.call(accounts[9])).div(
			BIG18,
		);
		const locked_balance_00_9 = new BigNumber(await staking_instance.lockedLiquidityOf.call(accounts[9])).div(
			BIG18,
		);
		...
		const staking_getRewardCall_1_arr = await staking_instance.getReward.call(COLLATERAL_FRAX_AND_FXS_OWNER, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		for (let j = 0; j < all_reward_tokens.length; j++) {
			...
			console.log(
				`accounts[1] earnings after 1 week [${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])}]: `,
				staking_earned_1.toString(),
			);
			const staking_getReward_1 = new BigNumber(staking_getRewardCall_1_arr[j]).div(BIG18);
			console.log(
				`accounts[1] getReward call after 1 week [${utilities.rewardTokenSymbolFromAddress(
					all_reward_tokens[j],
				)}]: `,
				staking_getReward_1.toString(),
			);

			// Make sure getReward() and earned() match
			assert(
				staking_earned_1.isEqualTo(staking_getReward_1),
				`${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])} getReward() and earned() mismatch`,
			);

			const staking_earned_9 = new BigNumber(staking_earned_9_arr[j]).div(BIG18);
			console.log(
				`accounts[9] earnings after 1 week [${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])}]: `,
				staking_earned_9.toString(),
			);
			...
			console.log(
				`Effective weekly reward at week 1 [${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])}]: `,
				reward_week_1[j].toString(),
			);
			console.log(
				`Effective yearly reward at week 1 [${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])}]: `,
				effective_yearly_reward_at_week_1.toString(),
			);

			const duration_reward_1 = new BigNumber(duration_reward_1_arr[j]).div(BIG18);
			console.log(
				`Expected yearly reward [${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])}]: `,
				duration_reward_1.multipliedBy(52.1429).toString(),
			);

			const reward_amount_this_week = new BigNumber(await staking_instance.rewardRates.call(j))
				.multipliedBy(604800)
				.div(BIG18)
				.toNumber();
			console.log(
				`reward per week from rewardRate (${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])}): `,
				reward_amount_this_week,
			);
		}

		// Note the FXS balance before the reward claims
		const fxs_bal_before_claim_wk_1 = new BigNumber(await fxs_instance.balanceOf(staking_instance.address))
			.div(BIG18)
			.toNumber();

		console.log('TRY WITHDRAWING AGAIN');
		console.log('[1] SHOULD SUCCEED, [9] SHOULD FAIL');
		await staking_instance.withdrawLocked(locked_stake_structs_1_0[0].kek_id, COLLATERAL_FRAX_AND_FXS_OWNER, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await expectRevert.unspecified(
			staking_instance.withdrawLocked(locked_stake_structs_9_0[0].kek_id, accounts[9], { from: accounts[9] }),
		...
		const fxs_bal_after_claim_wk_1 = new BigNumber(await fxs_instance.balanceOf(staking_instance.address))
			.div(BIG18)
			.toNumber();
		...
		assert(
			fxs_actually_emitted_wk_1 + early_earn >= reward_week_1[0].toNumber() * 0.99,
			'FXS actually emitted mismatches FXS earned() [underemission]',
		);
		assert(
			fxs_actually_emitted_wk_1 + early_earn <= reward_week_1[0].toNumber() * 1.01,
			'FXS actually emitted mismatches FXS earned() [overemission]',
		);

		// Print the reward rate
		let reward_amount_check2 = new BigNumber(await staking_instance.rewardRates.call(0))
			.multipliedBy(604800)
			.div(BIG18)
			.toNumber();
		...
		// Print out the gauge relative weights
		const gauge_rel_wgt_check_1 = new BigNumber(
			await frax_gauge_controller.gauge_relative_weight.call(staking_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		// Print the weekly emission
		const weekly_total_emission_check_1 = new BigNumber(await frax_gauge_controller.global_emission_rate.call())
			.multipliedBy(604800)
			.div(BIG18)
			.toNumber();
		...
		// Vote for this farm
		// 10000 = 100% of your weight. 1 = 0.01% of your weight
		await frax_gauge_controller.vote_for_gauge_weights(staking_instance.address, 1000, {
			from: ADDRESS_WITH_VEFXS,
		});
		...
		// Note the UNI POOL and FXS amount after staking
		const regular_balance_01_1 = new BigNumber(
			await staking_instance.lockedLiquidityOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const boosted_balance_01_1 = new BigNumber(
			await staking_instance.combinedWeightOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const locked_balance_01_1 = new BigNumber(
			await staking_instance.lockedLiquidityOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const regular_balance_01_9 = new BigNumber(await staking_instance.lockedLiquidityOf.call(accounts[9])).div(
			BIG18,
		);
		const boosted_balance_01_9 = new BigNumber(await staking_instance.combinedWeightOf.call(accounts[9])).div(
			BIG18,
		);
		const locked_balance_01_9 = new BigNumber(await staking_instance.lockedLiquidityOf.call(accounts[9])).div(
			BIG18,
		);
		...
		for (let j = 0; j < all_reward_tokens.length; j++) {
			...
			console.log(
				`accounts[1] earnings after 5 weeks [${utilities.rewardTokenSymbolFromAddress(
					all_reward_tokens[j],
				)}]: `,
				staking_earned_week5_1.toString(),
			);
			const staking_earned_week5_9 = new BigNumber(staking_earned_week5_9_arr[j]).div(BIG18);
			console.log(
				`accounts[9] earnings after 5 weeks [${utilities.rewardTokenSymbolFromAddress(
					all_reward_tokens[j],
				)}]: `,
				staking_earned_week5_9.toString(),
			);
			...
			console.log(
				`Effective weekly reward after 5 weeks [${utilities.rewardTokenSymbolFromAddress(
					all_reward_tokens[j],
				)}]: `,
				reward_week_5[j].div(4).toString(),
			);
			console.log(
				`Effective yearly reward after 5 weeks [${utilities.rewardTokenSymbolFromAddress(
					all_reward_tokens[j],
				)}]: `,
			...
			console.log(
				`Expected yearly reward [${utilities.rewardTokenSymbolFromAddress(all_reward_tokens[j])}]: `,
				duration_reward_week5.multipliedBy(52.1429).toString(),
			);
		...
		// Note the FXS balance before the reward claims
		const fxs_bal_before_claim_wk_5 = new BigNumber(await fxs_instance.balanceOf(staking_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await staking_instance.getReward(accounts[9], { from: accounts[9] });
		await expectRevert.unspecified(
			staking_instance.withdrawLocked(locked_stake_structs_1_0[1].kek_id, COLLATERAL_FRAX_AND_FXS_OWNER, {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			}),
		);
		...
		await staking_instance.withdrawLocked(locked_stake_structs_1_0[1].kek_id, COLLATERAL_FRAX_AND_FXS_OWNER, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		// Note the FXS balance after the reward claims
		const fxs_bal_after_claim_wk_5 = new BigNumber(await fxs_instance.balanceOf(staking_instance.address))
			.div(BIG18)
			.toNumber();
		...
		assert(
			fxs_actually_emitted_wk_5 >= reward_week_5[0].toNumber() * 0.99,
			'FXS actually emitted mismatches FXS earned() [underemission]',
		);
		assert(
			fxs_actually_emitted_wk_5 <= reward_week_5[0].toNumber() * 1.01,
			'FXS actually emitted mismatches FXS earned() [overemission]',
		);
		...
		const starting_bal_fxs_1 = new BigNumber(await fxs_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const starting_bal_fxs_9 = new BigNumber(await fxs_instance.balanceOf(accounts[9])).div(BIG18).toNumber();

		// Approve
		await pair_instance.approve(staking_instance.address, uni_pool_lock_boundary_check_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await pair_instance.approve(staking_instance.address, uni_pool_lock_boundary_check_amount, {
			from: accounts[9],
		...
		// Stake Locked
		// account[1]
		await staking_instance.stakeLocked(uni_pool_lock_boundary_check_amount, 10 * 86400, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const ending_bal_fxs_1 = new BigNumber(await fxs_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		...
		console.log(
			'accounts[1] staked lockedLiquidityOf <BEFORE>:',
			new BigNumber(await staking_instance.lockedLiquidityOf(COLLATERAL_FRAX_AND_FXS_OWNER))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
			'accounts[1] staked combinedWeightOf <BEFORE>:',
			new BigNumber(await staking_instance.combinedWeightOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18).toNumber(),
		);
		console.log(
			'accounts[9] staked lockedLiquidityOf <BEFORE>:',
			new BigNumber(await staking_instance.lockedLiquidityOf(accounts[9])).div(BIG18).toNumber(),
		);
		console.log(
			'accounts[9] staked combinedWeightOf <BEFORE>:',
			new BigNumber(await staking_instance.combinedWeightOf(accounts[9])).div(BIG18).toNumber(),
		);
		...
		const vefxs_multiplier_1_pre_proxy = new BigNumber(
			await staking_instance.veFXSMultiplier.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const vefxs_multiplier_9_pre_proxy = new BigNumber(
			await staking_instance.veFXSMultiplier.call(accounts[9]),
		).div(BIG18);
		const min_vefxs_whale_pre_proxy = new BigNumber(
			await staking_instance.minVeFXSForMaxBoostProxy.call(accounts[4]),
		).div(BIG18);
		const lp_whale_pre_proxy = new BigNumber(await staking_instance.proxy_lp_balances.call(accounts[4])).div(BIG18);
		const max_lp_whale_pre_proxy = new BigNumber(await staking_instance.maxLPForMaxBoost.call(accounts[4])).div(
			BIG18,
		);
		...
		const vefxs_multiplier_1_post_proxy = new BigNumber(
			await staking_instance.veFXSMultiplier.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const vefxs_multiplier_9_post_proxy = new BigNumber(
			await staking_instance.veFXSMultiplier.call(accounts[9]),
		).div(BIG18);
		const min_vefxs_whale_post_proxy = new BigNumber(
			await staking_instance.minVeFXSForMaxBoostProxy.call(accounts[4]),
		).div(BIG18);
		const lp_whale_post_proxy = new BigNumber(await staking_instance.proxy_lp_balances.call(accounts[4])).div(
			BIG18,
		);
		const max_lp_whale_post_proxy = new BigNumber(await staking_instance.maxLPForMaxBoost.call(accounts[4])).div(
			BIG18,
		);
		...
		assert(
			vefxs_multiplier_9_post_proxy.isGreaterThanOrEqualTo(vefxs_multiplier_9_pre_proxy),
			`Proxying should have boosted the multiplier`,
		);
		...
		const vefxs_multiplier_1_post_proxy_addl = new BigNumber(
			await staking_instance.veFXSMultiplier.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const vefxs_multiplier_9_post_proxy_addl = new BigNumber(
			await staking_instance.veFXSMultiplier.call(accounts[9]),
		).div(BIG18);
		const min_vefxs_whale_post_proxy_addl = new BigNumber(
			await staking_instance.minVeFXSForMaxBoostProxy.call(accounts[4]),
		).div(BIG18);
		const lp_whale_post_proxy_addl = new BigNumber(await staking_instance.proxy_lp_balances.call(accounts[4])).div(
			BIG18,
		);
		const max_lp_whale_post_proxy_addl = new BigNumber(
			await staking_instance.maxLPForMaxBoost.call(accounts[4]),
		).div(BIG18);
		...
		assert(
			min_vefxs_whale_post_proxy_addl.isGreaterThan(min_vefxs_whale_post_proxy),
			`minVeFXSForMaxBoostProxy should have increased`,
		);
		...
		const vefxs_multiplier_1_post_proxy_addl_max = new BigNumber(
			await staking_instance.veFXSMultiplier.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const vefxs_multiplier_9_post_proxy_addl_max = new BigNumber(
			await staking_instance.veFXSMultiplier.call(accounts[9]),
		).div(BIG18);
		const min_vefxs_whale_post_proxy_addl_max = new BigNumber(
			await staking_instance.minVeFXSForMaxBoostProxy.call(accounts[4]),
		).div(BIG18);
		const lp_whale_post_proxy_addl_max = new BigNumber(
			await staking_instance.proxy_lp_balances.call(accounts[4]),
		).div(BIG18);
		const max_lp_whale_post_proxy_addl_max = new BigNumber(
			await staking_instance.maxLPForMaxBoost.call(accounts[4]),
		).div(BIG18);
		...
		assert(
			vefxs_multiplier_1_post_proxy_addl_max.isLessThan(vefxs_multiplier_1_post_proxy_addl),
			`veFXS multiplier should have decreased`,
		);
		assert(
			vefxs_multiplier_9_post_proxy_addl_max.isLessThan(vefxs_multiplier_9_post_proxy_addl),
			`veFXS multiplier should have decreased`,
		);
		...
		const vefxs_multiplier_1_post_proxy_disallow = new BigNumber(
			await staking_instance.veFXSMultiplier.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const vefxs_multiplier_9_post_proxy_disallow = new BigNumber(
			await staking_instance.veFXSMultiplier.call(accounts[9]),
		).div(BIG18);
		const min_vefxs_whale_post_proxy_disallow = new BigNumber(
			await staking_instance.minVeFXSForMaxBoostProxy.call(accounts[4]),
		).div(BIG18);
		const lp_whale_post_proxy_disallow = new BigNumber(
			await staking_instance.proxy_lp_balances.call(accounts[4]),
		).div(BIG18);
		const max_lp_whale_post_proxy_disallow = new BigNumber(
			await staking_instance.maxLPForMaxBoost.call(accounts[4]),
		).div(BIG18);
		...
		assert(
			vefxs_multiplier_1_post_proxy_disallow.isEqualTo(vefxs_multiplier_1_pre_proxy),
			`Multiplier [1] should be back to normal`,
		);
		assert(
			vefxs_multiplier_9_post_proxy_disallow.isEqualTo(vefxs_multiplier_9_pre_proxy),
			`Multiplier [9] should be back to normal`,
		);
		...
		await staking_instance.lockAdditional(add_more_before.kek_id, uni_pool_lock_boundary_check_amount, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
	...
	it('Communal / Token Manager Tests', async () => {
		...
		await staking_instance.setRewardVars(
			staking_reward_tokens_addresses[1],
			1000,
		...
		for (let j = 0; j < staking_reward_tokens_addresses.length; j++) {
			...
			const token_manager_address = await staking_instance.rewardManagers.call(
				staking_reward_tokens_addresses[j],
			);
			...
			await staking_instance.setRewardVars(rew_tkn_addr, 0, gauge_ctr_add_to_use, gauge_rew_dist_add_to_use, {
				from: token_manager_address,
			});
			...
			assert(
				tkn_bal_before_rec.minus(tkn_bal_after_rec).isEqualTo(test_recovery_amount),
				'Reward token not recovered',
			);

			console.log('Change the token manager');
			await staking_instance.changeTokenManager(rew_tkn_addr, COLLATERAL_FRAX_AND_FXS_OWNER, {
				from: token_manager_address,
			});
	...
	it('Migration Staking / Withdrawal Tests', async () => {
		...
		// Print the balance
		console.log(
			'accounts[1] ERC20 balanceOf LP:',
			new BigNumber(await pair_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18).toNumber(),
		);

		// Stake Locked
		await pair_instance.approve(staking_instance.address, stake_amt_locked, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const locked_liq_before0 = new BigNumber(
			await staking_instance.lockedLiquidityOf(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		const combined_weight_before0 = new BigNumber(
			await staking_instance.combinedWeightOf(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		await staking_instance.migrator_withdraw_locked(COLLATERAL_FRAX_AND_FXS_OWNER, locked_stake_structs[3].kek_id, {
			from: MIGRATOR_ADDRESS,
		});
		console.log(
			`Migrator (accounts[10]) withdrew ${withdraw_locked_amt.div(
				BIG18,
			)} (E18) locked LP tokens from accounts[1]`,
		);
		console.log(
			'Migrator (accounts[10]) ERC20 balanceOf:',
			new BigNumber(await pair_instance.balanceOf(MIGRATOR_ADDRESS)).div(BIG18).toNumber(),
		);
		const locked_liq_after0 = new BigNumber(await staking_instance.lockedLiquidityOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const combined_weight_after0 = new BigNumber(
			await staking_instance.combinedWeightOf(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		...
		const locked_liq_before1 = new BigNumber(
			await staking_instance.lockedLiquidityOf(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		const combined_weight_before1 = new BigNumber(
			await staking_instance.combinedWeightOf(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
		...
		let block_time_current_1 = (await time.latest()).toNumber();
		await staking_instance.migrator_stakeLocked_for(
			COLLATERAL_FRAX_AND_FXS_OWNER,
			proxy_stake_lock_amt,
			28 * 86400,
			block_time_current_1,
			{ from: MIGRATOR_ADDRESS },
		);
		...
		const locked_liq_after1 = new BigNumber(await staking_instance.lockedLiquidityOf(COLLATERAL_FRAX_AND_FXS_OWNER))
			.div(BIG18)
			.toNumber();
		const combined_weight_after1 = new BigNumber(
			await staking_instance.combinedWeightOf(COLLATERAL_FRAX_AND_FXS_OWNER),
		)
			.div(BIG18)
			.toNumber();
	...
	it('Migration Fail Tests ', async () => {
		...
		await expectRevert(
			staking_instance.migrator_withdraw_locked(COLLATERAL_FRAX_AND_FXS_OWNER, locked_stake_structs[2].kek_id, {
				from: MIGRATOR_ADDRESS,
			}),
			'Not in migration',
		);
		...
		await expectRevert(
			staking_instance.migrator_stakeLocked_for(
				COLLATERAL_FRAX_AND_FXS_OWNER,
				test_amount_1,
				28 * 86400,
				block_time_current_2,
				{ from: MIGRATOR_ADDRESS },
			),
			'Not in migration',
		);
	...
	it('Proxy Tests', async () => {
		...
		staking_instance.toggleValidVeFXSProxy(GOVERNOR_GUARDIAN_ADDRESS, { from: STAKING_OWNER }),
			console.log(
				'--------- TRY ALLOWING A VALID PROXY FOR A STAKER BEFORE THE PROXY TOGGLED THEM FIRST ---------',
			);
		await expectRevert(
