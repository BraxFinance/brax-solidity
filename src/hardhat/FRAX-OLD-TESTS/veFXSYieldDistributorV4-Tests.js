...
contract('veFXSYieldDistributorV4-Tests', async (accounts) => {
	...
	it('Initialization related', async () => {
		...
		await fxs_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('250000e18'), {
		...
		await veFXSYieldDistributor_instance.toggleRewardNotifier(COLLATERAL_FRAX_AND_FXS_OWNER, {
			from: STAKING_OWNER,
		});
		...
		await fxs_instance.approve(veFXSYieldDistributor_instance.address, yield_week_1_e18, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await veFXSYieldDistributor_instance.notifyRewardAmount(yield_week_1_e18, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		console.log(
			'1st week Yield Rate (1st half): ',
			new BigNumber(await veFXSYieldDistributor_instance.getYieldForDuration.call()).div(BIG18).toNumber(),
		);
	...
	it('Normal yield', async () => {
		...
		// Note accounts[1]
		const vefxs_at_1st_checkpoint_1 = new BigNumber(
			await veFXS_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_at_1st_checkpoint_1 = new BigNumber(
			await fxs_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_in_vefxs_at_1st_checkpoint_1 = new BigNumber(
			(await veFXS_instance.locked.call(COLLATERAL_FRAX_AND_FXS_OWNER)).amount,
		).div(BIG18);
		const yield_at_1st_checkpoint_1_FXS = new BigNumber(
			await veFXSYieldDistributor_instance.yields.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		...
		console.log(
			'accounts[1] userVeFXSCheckpointed:',
			new BigNumber(await veFXSYieldDistributor_instance.userVeFXSCheckpointed(COLLATERAL_FRAX_AND_FXS_OWNER))
				.div(BIG18)
				.toNumber(),
		);
		console.log('');
		...
		const yield_at_1st_checkpoint_9_FXS = new BigNumber(
			await veFXSYieldDistributor_instance.yields(accounts[9]),
		).div(BIG18);
		...
		console.log(
			'accounts[9] userVeFXSCheckpointed:',
			new BigNumber(await veFXSYieldDistributor_instance.userVeFXSCheckpointed(accounts[9]))
				.div(BIG18)
				.toNumber(),
		);
		console.log('');
		...
		let distributor_lastTimeYieldApplicable = new BigNumber(
			await veFXSYieldDistributor_instance.lastTimeYieldApplicable.call(),
		);
		...
		// Note the total lastTimeYieldApplicable
		distributor_lastTimeYieldApplicable = new BigNumber(
			await veFXSYieldDistributor_instance.lastTimeYieldApplicable.call(),
		);
		console.log('Distributor lastTimeYieldApplicable():', distributor_lastTimeYieldApplicable.toNumber());

		// Note the total veFXS supply stored
		const distributor_total_vefxs_supply_Stored = new BigNumber(
			await veFXSYieldDistributor_instance.totalVeFXSSupplyStored.call(),
		).div(BIG18);
		...
		const fractionParticipating = new BigNumber(await veFXSYieldDistributor_instance.fractionParticipating.call())
			.div(BIG6)
			.toNumber();
		...
		console.log(
			`Expected weekly yield [FXS], accounting for ${pct_participating}% participation: `,
			expected_weekly_1.toNumber(),
		);

		// Check to make the sure the yields are within expected values
		assert(
			effective_weekly_yield_at_week_1_fxs.toNumber() >= expected_weekly_1.toNumber() * 0.99,
			'Effective weekly yield < expected weekly yield [underemission]',
		);
		assert(
			effective_weekly_yield_at_week_1_fxs.toNumber() <= expected_weekly_1.toNumber() * 1.01,
			'Effective weekly yield > Expected weekly yield [overemission]',
		);
		...
		const veFXS_post_yield_1 = new BigNumber(
			await veFXS_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_post_yield_1 = new BigNumber(await fxs_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER)).div(
			BIG18,
		);
		...
		await fxs_instance.approve(veFXSYieldDistributor_instance.address, yield_week_2_e18, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await veFXSYieldDistributor_instance.notifyRewardAmount(yield_week_2_e18, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		console.log(
			'2nd week Yield Rate: ',
		...
		console.log(
			'accounts[9] staking earned() (THIS WEEK) [FXS]:',
			staking_part2_earned_9_fxs.toNumber() - staking_wk1_earned_9_fxs.toNumber(),
		);
		console.log('accounts[9] staking earned() (TOTAL) [FXS]:', staking_part2_earned_9_fxs.toNumber());
		console.log('');

		const veFXS_2nd_time_balance_1 = new BigNumber(
			await veFXS_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		...
		console.log(
			...
			acc_9_FXS_balance_after.minus(fxs_at_1st_checkpoint_9).toNumber(),
		);

		console.log('====================================================================');
		const current_timestamp_3rd = new BigNumber(await time.latest()).toNumber();
		const veFXS_timeleft =
			(await veFXS_instance.locked__end.call(COLLATERAL_FRAX_AND_FXS_OWNER)) - current_timestamp_3rd;
		...
		const veFXS_3rd_time_balance_1 = new BigNumber(
			await veFXS_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_3rd_time_balance_1 = new BigNumber(
			await fxs_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_in_vefxs_3rd_time_balance_1 = new BigNumber(
			(await veFXS_instance.locked.call(COLLATERAL_FRAX_AND_FXS_OWNER)).amount,
		).div(BIG18);
		...
		assert(
			yield_earned_3rd_time_1_fxs.toNumber() == staking_part2_earned_1_fxs.toNumber(),
			'Should not have accrued extra FXS',
		);
		...
		await fxs_instance.approve(veFXSYieldDistributor_instance.address, yield_week_last_e18, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await veFXSYieldDistributor_instance.notifyRewardAmount(yield_week_last_e18, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		console.log(
			'Last week Yield Rate: ',
			new BigNumber(await veFXSYieldDistributor_instance.getYieldForDuration.call()).div(BIG18).toNumber(),
		);
		...
		const veFXS_4th_time_balance_1 = new BigNumber(
			await veFXS_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_4th_time_balance_1 = new BigNumber(
			await fxs_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_in_vefxs_4th_time_balance_1 = new BigNumber(
			(await veFXS_instance.locked.call(COLLATERAL_FRAX_AND_FXS_OWNER)).amount,
		).div(BIG18);
		...
		const veFXS_5th_time_balance_1 = new BigNumber(
			await veFXS_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_5th_time_balance_1 = new BigNumber(
			await fxs_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_in_vefxs_5th_time_balance_1 = new BigNumber(
			(await veFXS_instance.locked.call(COLLATERAL_FRAX_AND_FXS_OWNER)).amount,
		).div(BIG18);
