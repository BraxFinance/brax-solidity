...
describe('FraxswapRange AMM', function () {
	...
	it('initial mint', async function () {
		...
		expect(await contracts.rangePair.balanceOf(owner.address)).to.equal(
			BigInt('200000000000000000000') - BigInt(1000),
		);
	});
	...
	it('burn', async function () {
		...
		expect(await contracts.rangePair.balanceOf(owner.address)).to.equal(
			BigInt('100000000000000000000') - BigInt(1000),
		);
		expect(await contracts.token0.balanceOf(owner.address)).to.equal(
			BigInt(balance0Before) + BigInt('50000000000000000000'),
		);
		expect(await contracts.token1.balanceOf(owner.address)).to.equal(
			BigInt(balance1Before) + BigInt('50000000000000000000'),
		);
	});
	...
	it('swap at edge', async function () {
		...
		await expect(contracts.rangePair.swap(0, expectedOutput + BigInt(1), user1.address, [])).to.be.revertedWith(
			'UniswapV2: K',
		);
	...
	it('swap over the edge', async function () {
		...
		await expect(contracts.rangePair.swap(0, '100000000000000000001', user1.address, [])).to.be.revertedWith(
	...
	it('swap token1 -> token0', async function () {
		...
		// Swap fails if you ask for too much
		await expect(contracts.rangePair.swap(expectedOutput + BigInt(1), 0, user1.address, [])).to.be.revertedWith(
			'UniswapV2: K',
		);
				...
				for (var c = 0; c < amounts.length; c++) {
					for (var d = 0; d < amounts.length; d++) {
						var contracts = await setupContracts(
							(amounts[c] * PRECISION) / amounts[d],
							(amounts[b] * PRECISION) / amounts[d],
							30,
						);
						// Add initial liquidity
						...
						if (expectedOutput > 0 && expectedOutput + BigInt(1) <= actualBalances1) {
							...
							// Swap fails if you ask for too much
							await expect(
								contracts.rangePair.swap(0, expectedOutput + BigInt(1), user1.address, []),
...
function getExpectedOutput(_reserve0, _reserve1, tradeAmount, fee) {
	return (
		(tradeAmount * BigInt(10000 - fee) * BigInt(_reserve1)) /
		(BigInt(_reserve0) * BigInt(10000) + tradeAmount * BigInt(10000 - fee))
	);
}
