...
describe('FPI PermissionedSend', function () {
	...
	it('setOperator', async function () {
		...
		await expect(contracts.permissionedSend.connect(user3).setOperator(user1.address)).to.be.revertedWith(
	...
	it('addTarget', async function () {
		...
		await expect(
			contracts.permissionedSend.connect(user3).addTarget(user2.address, contracts.fpiToken.address),
		).to.be.revertedWith('Ownable: caller is not the owner');

		// Check if not set before
		expect(await contracts.permissionedSend.sendAllowed(user2.address, contracts.fpiToken.address)).to.be.equal(
			false,
		);
	...
	it('removeTarget', async function () {
		...
		await expect(
			contracts.permissionedSend.connect(user3).removeTarget(user2.address, contracts.fpiToken.address),
		).to.be.revertedWith('Ownable: caller is not the owner');
	...
	it('sendERC20', async function () {
		...
		await expect(
			contracts.permissionedSend.connect(user3).sendERC20(user2.address, contracts.fpiToken.address, 1000000),
		).to.be.revertedWith('PermissionedSend:Caller is not the operator');

		// Can only send approved token
		await expect(
			contracts.permissionedSend.connect(user1).sendERC20(user2.address, contracts.fraxToken.address, 1000000),
		).to.be.revertedWith('Send not allowed');
	...
	it('sendETH', async function () {
		...
		await expect(contracts.permissionedSend.connect(user3).sendETH(user2.address, 1000000)).to.be.revertedWith(
			'PermissionedSend:Caller is not the operator',
		);
