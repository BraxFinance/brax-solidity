...
contract('FraxPoolInvestorForV2-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		const fxs_balance_before = new BigNumber(
			await fxs_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG18);
		const usdc_balance_before = new BigNumber(
			await usdc_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		console.log('FXS before: ', fxs_balance_before.toNumber());
		console.log('USDC before: ', usdc_balance_before.toNumber());

		await fraxPoolInvestorForV2_instance.mintRedeemPart1(new BigNumber('15000e18'), {
		...
		const borrowed_balance = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		...
		await fraxPoolInvestorForV2_instance.burnFXS(fxs_balance_after.multipliedBy(BIG18), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const fxs_balance_before_recollat = new BigNumber(
			await fxs_instance.balanceOf.call(process.env.FRAX_ONE_ADDRESS),
		).div(BIG18);
		console.log('fxs_balance_before_recollat: ', fxs_balance_before_recollat.toNumber());

		await usdc_instance.approve(pool_instance_USDC.address, new BigNumber('100e6'), {
			from: process.env.FRAX_ONE_ADDRESS,
		});
		await pool_instance_USDC.recollateralizeFRAX(new BigNumber('100e6'), 0, { from: process.env.FRAX_ONE_ADDRESS });

		const fxs_balance_after_recollat = new BigNumber(
			await fxs_instance.balanceOf.call(process.env.FRAX_ONE_ADDRESS),
		).div(BIG18);
		console.log('fxs_balance_after_recollat: ', fxs_balance_after_recollat.toNumber());

		console.log('======================INVEST INTO yVault======================');
		await fraxPoolInvestorForV2_instance.yDepositUSDC(usdc_balance_after.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const yUSD_balance = new BigNumber(
			await yUSDC_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		console.log('yUSDC balance: ', yUSD_balance.toNumber());
		await fraxPoolInvestorForV2_instance.yWithdrawUSDC(yUSD_balance.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await expectRevert.unspecified(
			fraxPoolInvestorForV2_instance.mintRedeemPart1(new BigNumber('10000000e18'), {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			}),
		);

		console.log('======================INVEST INTO aUSDC======================');
		await fraxPoolInvestorForV2_instance.aaveDepositUSDC(usdc_balance_after_withdrawal_y.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const aUSDC_balance = new BigNumber(
			await aUSDC_token_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		console.log('aUSDC balance: ', aUSDC_balance.toNumber());
		await fraxPoolInvestorForV2_instance.aaveWithdrawUSDC(aUSDC_balance.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await fraxPoolInvestorForV2_instance.compoundMint_cUSDC(usdc_balance_after_withdrawal_a.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const cUSDC_balance = new BigNumber(
			await cUSDC_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG8);
		console.log('cUSDC balance: ', cUSDC_balance.toNumber());
		await fraxPoolInvestorForV2_instance.compoundRedeem_cUSDC(cUSDC_balance.multipliedBy(BIG8), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const usdc_balance_before_emergency = new BigNumber(
			await usdc_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		console.log('USDC balance before emergency: ', usdc_balance_before_emergency.toNumber());
		await fraxPoolInvestorForV2_instance.emergencyrecoverERC20(usdc_instance.address, new BigNumber('1e3'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const usdc_balance_after_emergency = new BigNumber(
			await usdc_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		console.log('USDC balance after emergency: ', usdc_balance_after_emergency.toNumber());
		console.log(
			'USDC withdrawn: ',
			usdc_balance_before_emergency.toNumber() - usdc_balance_after_emergency.toNumber(),
		);

		console.log('=====================GIVE COLLATERAL BACK=====================');

		const usdc_balance_before_giving_back = new BigNumber(
			await usdc_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		console.log('USDC balance before giving back: ', usdc_balance_before_giving_back.toNumber());
		await fraxPoolInvestorForV2_instance.giveCollatBack(usdc_balance_before_giving_back.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const usdc_balance_after_giving_back = new BigNumber(
			await usdc_instance.balanceOf.call(fraxPoolInvestorForV2_instance.address),
		).div(BIG6);
		const borrowed_balance_after = new BigNumber(await fraxPoolInvestorForV2_instance.borrowed_balance.call()).div(
			BIG6,
		);
		const borrowed_historical = new BigNumber(await fraxPoolInvestorForV2_instance.borrowed_historical.call()).div(
			BIG6,
		);
		const paid_back_historical = new BigNumber(
			await fraxPoolInvestorForV2_instance.paid_back_historical.call(),
		).div(BIG6);
