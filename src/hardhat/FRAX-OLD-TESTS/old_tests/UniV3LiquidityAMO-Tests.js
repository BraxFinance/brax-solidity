...
contract('UniV3LiquidityAMO Tests', async (accounts) => {
	...
	beforeEach(async () => {
		...
		univ3_position_manager_instance = await INonfungiblePositionManager.at(
	...
	it('Main test', async () => {
		...
		await frax_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('150000e18'), {
		...
		await usdc_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('150000e6'), {
		...
		console.log(
			'Uni v3 AMO [collatDollarBalance()]:',
			new BigNumber(await univ3_liquidity_amo_instance.collatDollarBalance()).div(BIG18).toNumber(),
		);
		...
		await univ3_liquidity_amo_instance.setSafetyParams(
			new BigNumber('820000'),
			new BigNumber('500000e18'),
			new BigNumber('500000e6'),
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		);
		console.log(
			'Uni v3 AMO [min_cr]:',
			new BigNumber(await univ3_liquidity_amo_instance.min_cr()).div(BIG6).toNumber(),
		);
		console.log(
			'Uni v3 AMO [mint_cap]:',
			new BigNumber(await univ3_liquidity_amo_instance.mint_cap()).div(BIG18).toNumber(),
		);
		console.log(
			'Uni v3 AMO [collat_borrow_cap]:',
			new BigNumber(await univ3_liquidity_amo_instance.min_cr()).div(BIG6).toNumber(),
		);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold(
					'====================== Do 100k worth of FRAX mintRedeem() to get USDC into Uni v3 AMO =====================',
				),
		);
		// Refresh oracle
		try {
		...
		await univ3_liquidity_amo_instance.mintRedeemPart1(new BigNumber('100000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await time.increase(20);
		await time.advanceBlock();
		...
		console.log(
			'Uni v3 AMO [USDC balance]:',
			new BigNumber(await usdc_instance.balanceOf(univ3_liquidity_amo_instance.address)).div(BIG6).toNumber(),
		);
		console.log(
			'Uni v3 AMO [FRAX balance]:',
			new BigNumber(await frax_instance.balanceOf(univ3_liquidity_amo_instance.address)).div(BIG18).toNumber(),
		);

		console.log(
			chalk.hex('#ff8b3d').bold('====================== 100k FRAX into the Uni v3 AMO ====================='),
		);
		await univ3_liquidity_amo_instance.mintFRAXForInvestments(new BigNumber('100000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		console.log(
			'Uni v3 AMO [collatDollarBalance()]:',
		...
		// UDSC
		console.log(
			'Uni v3 AMO [borrowed_collat_historical]:',
			new BigNumber(await univ3_liquidity_amo_instance.borrowed_collat_historical()).div(BIG6).toNumber(),
		);
		console.log(
			'Uni v3 AMO [collateralBalance()]:',
			new BigNumber(await univ3_liquidity_amo_instance.collateralBalance()).div(BIG6).toNumber(),
		);

		// FRAX
		console.log(
			'Uni v3 AMO [minted_sum_historical]:',
			new BigNumber(await univ3_liquidity_amo_instance.minted_sum_historical()).div(BIG18).toNumber(),
		);
		console.log(
			'Uni v3 AMO [mintedBalance()]:',
			new BigNumber(await univ3_liquidity_amo_instance.mintedBalance()).div(BIG18).toNumber(),
		);

		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('====================== Testing Uni v3 NonfungiblePositionManager ====================='),
		);
		console.log('\n>approve FRAX and USDC for Uni v3 NonfungiblePositionManager');
		await frax_instance.approve(univ3_position_manager_instance.address, new BigNumber('1e42'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await usdc_instance.approve(univ3_position_manager_instance.address, new BigNumber('1e30'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log(
			'accounts[1] [USDC balance]:',
			new BigNumber(await usdc_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG6).toNumber(),
		);
		console.log(
			'accounts[1] [FRAX balance]:',
			new BigNumber(await frax_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18).toNumber(),
		);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('====================== Mint 10k/10k Uni v3 FRAX-USDC Position ====================='),
		);
		...
		console.log(
			'AMO # of Uni v3 Positions:',
			new BigNumber(
				await univ3_position_manager_instance.balanceOf(univ3_liquidity_amo_instance.address),
			).toNumber(),
		);
		...
		console.log(chalk.hex('#ff8b3d').bold('====================== CHECK ACCOUNTING ====================='));
		const token_id_0 = (
			await univ3_position_manager_instance.tokenOfOwnerByIndex(univ3_liquidity_amo_instance.address, 0)
		).toNumber();
		...
		console.log(
			'positions_mapping[token_id].liquidity:',
			new BigNumber((await univ3_liquidity_amo_instance.positions_mapping(token_id_0)).liquidity).toNumber(),
		);
		console.log(
			'Uni v3 AMO [collatDollarBalance()]:',
			new BigNumber(await univ3_liquidity_amo_instance.collatDollarBalance()).div(BIG18).toNumber(),
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold(
					'====================== Withdraw all liquidity from the minted NFT position from uni v3 =====================',
				),
		);
		...
		console.log(
			'Uni v3 AMO [DAI balance]:',
			new BigNumber(await daiInstance.balanceOf(univ3_liquidity_amo_instance.address)).div(BIG18).toNumber(),
		);

		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('====================== Mint 5k/5k Uni v3 FRAX-DAI Position ====================='),
		);
		await univ3_liquidity_amo_instance.mint(
			daiInstance.address,
		...
		console.log(chalk.hex('#ff8b3d').bold('====================== CHECK ACCOUNTING ====================='));
		const token_id_1 = (
			await univ3_position_manager_instance.tokenOfOwnerByIndex(univ3_liquidity_amo_instance.address, 1)
		).toNumber();
		...
		console.log(
			'positions_mapping[token_id].liquidity:',
			new BigNumber((await univ3_liquidity_amo_instance.positions_mapping(token_id_1)).liquidity).toNumber(),
		);
		console.log(
			'Uni v3 AMO [collatDollarBalance()]:',
			new BigNumber(await univ3_liquidity_amo_instance.collatDollarBalance()).div(BIG18).toNumber(),
		...
		console.log(
			'Custodian # of Uni v3 Positions:',
			new BigNumber(await univ3_position_manager_instance.balanceOf(accounts[8])).toNumber(),
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('====================== Recover empty FRAX-USDC NFT ====================='),
		);
		await univ3_liquidity_amo_instance.recoverERC721(
			CONTRACT_ADDRESSES.ethereum.uniswap_v3.NonfungiblePositionManager,
			token_id_0,
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		);
		...
		console.log('\n>approve router');
		console.log(
			'accounts[1] [DAI balance]:',
			new BigNumber(await daiInstance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18).toNumber(),
		);
		console.log(
			'accounts[1] [FRAX balance]:',
			new BigNumber(await frax_instance.balanceOf(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18).toNumber(),
		);
		await daiInstance.approve(swapRouterInstance.address, new BigNumber('1e42'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await frax_instance.approve(swapRouterInstance.address, new BigNumber('1e42'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('====================== Swap 2k FRAX for DAI to accrue some fees to LPs ====================='),
		);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('====================== Swap 1k DAI for FRAX to accrue some fees to LPs ====================='),
		);
		...
		console.log(
			'custodian [DAI balance]:',
			new BigNumber(await daiInstance.balanceOf(INVESTOR_CUSTODIAN_ADDRESS)).div(BIG18).toNumber(),
		);
		console.log(
			'custodian [FRAX balance]:',
			new BigNumber(await frax_instance.balanceOf(INVESTOR_CUSTODIAN_ADDRESS)).div(BIG18).toNumber(),
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('====================== TEST USDT safeApprove PROBLEM ====================='),
		);
		await hre.network.provider.request({
			method: 'hardhat_impersonateAccount',
		...
		await usdt_instance.transfer(univ3_liquidity_amo_instance.address, new BigNumber('10000e6'), {
			from: ADDRESS_WITH_USDT,
		});
		...
		await univ3_liquidity_amo_instance.approveTarget(swapRouterInstance.address, usdt_instance.address, 0, true, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		// Add USDT as collateral
		await univ3_liquidity_amo_instance.addCollateral(usdt_instance.address, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'Uni v3 AMO [FEI balance]:',
			new BigNumber(await fei_instance.balanceOf.call(univ3_liquidity_amo_instance.address))
				.div(BIG18)
				.toNumber(),
		);

		console.log('\n>Approve FEI from the Uni v3 AMO');
		await univ3_liquidity_amo_instance.approveTarget(
			uniswapV3PositionsNFTInstance.address,
			fei_instance.address,
			new BigNumber('1e42'),
			true,
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		...
		console.log(
			'collateral_address:',
			(await univ3_liquidity_amo_instance.positions_mapping(token_id_fei)).collateral_address,
		);
		console.log(
			'tickLower:',
			new BigNumber((await univ3_liquidity_amo_instance.positions_mapping(token_id_fei)).tickLower).toNumber(),
		);
		console.log(
			'tickUpper::',
			new BigNumber((await univ3_liquidity_amo_instance.positions_mapping(token_id_fei)).tickUpper).toNumber(),
		);
		console.log(
			'liquidity:',
			new BigNumber((await univ3_liquidity_amo_instance.positions_mapping(token_id_fei)).liquidity).toNumber(),
		);
		...
		const amount0 = new BigNumber(
			await testing_instance.getAmount0ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, liquidity_fei),
		);
		const amount1 = new BigNumber(
			await testing_instance.getAmount1ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, liquidity_fei),
		);
		...
		await ust_instance.transfer(univ3_liquidity_amo_instance.address, new BigNumber('1000000e18'), {
			from: ADDRESS_WITH_UST,
		});
		...
		// );
		console.log(
			'Uni v3 AMO UST balance:',
			new BigNumber(await ust_instance.balanceOf.call(univ3_liquidity_amo_instance.address))
				.div(BIG18)
				.toNumber(),
		);
