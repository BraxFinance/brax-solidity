...
contract('MSIGHelper-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await frax_amo_minter_instance.mintFraxForAMO(msig_helper_instance.address, new BigNumber('100250e18'), {
		...
		await frax_amo_minter_instance.mintFxsForAMO(msig_helper_instance.address, new BigNumber('100250e18'), {
			from: process.env.FRAX_ONE_ADDRESS,
		});
		...
		await frax_amo_minter_instance.giveCollatToAMO(msig_helper_instance.address, new BigNumber('100000e6'), {
			from: process.env.FRAX_ONE_ADDRESS,
		});
		...
		console.log(
			'FRAX TokenType:',
			new BigNumber(await msig_helper_instance.getTokenType.call(frax_instance.address)).toNumber(),
		);
		await msig_helper_instance.lend(frax_instance.address, new BigNumber('1000e18'), {
		...
		console.log(
			'FXS TokenType:',
			new BigNumber(await msig_helper_instance.getTokenType.call(fxs_instance.address)).toNumber(),
		);
		await msig_helper_instance.lend(fxs_instance.address, new BigNumber('1000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'USDC TokenType:',
			new BigNumber(await msig_helper_instance.getTokenType.call(usdc_instance.address)).toNumber(),
		);
		await msig_helper_instance.lend(usdc_instance.address, new BigNumber('1000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(msig_helper_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		...
		await frax_amo_minter_instance.setAMOCorrectionOffsets(msig_helper_instance.address, 0, 0, {
			from: process.env.FRAX_ONE_ADDRESS,
		});
		...
		console.log(
			...
			new BigNumber((await msig_helper_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
