...
contract('FXSRewards-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await fxs_instance.transfer(veFXS_instance.address, new BigNumber('100e18'), {
			from: ORIGINAL_FRAX_ONE_ADDRESS,
		});

		console.log(
			'FXSRewards contract fxsDollarValueStaked():',
			new BigNumber(await FXSRewards_instance.fxsDollarValueStaked.call()).div(BIG18).toNumber(),
		);
	});
});
