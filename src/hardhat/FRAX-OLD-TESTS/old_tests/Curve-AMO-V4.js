...
contract('Curve AMO V4 Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await fxs_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('500000e18'), {
		...
		await frax_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('1000e18'), {
		...
		await usdc_real_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('10000000e6'), {
			from: ADDRESS_WITH_USDC_2,
		});
		...
		// Give the COLLATERAL_FRAX_AND_FXS_OWNER address a lot of real USDC
		await usdc_real_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('120000000e6'), {
			from: ADDRESS_WITH_USDC_3,
		});
		...
		await usdt_real_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('120000000e6'), {
		...
		await daiInstance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('20000000e18'), {
			from: '0xC2C5A77d9f434F424Df3d39de9e90d95A0Df5Aca',
		});
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold("======================MAKE SURE FRAX DIDN'T BRICK [TEST A REDEEM]====================="),
		);
		...
		const frax_balance_before_redeem = new BigNumber(
			await frax_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_balance_before_redeem = new BigNumber(
			await fxs_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const usdc_balance_before_redeem = new BigNumber(
			await usdc_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG6);
		...
		const frax_balance_after_redeem = new BigNumber(
			await frax_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const fxs_balance_after_redeem = new BigNumber(
			await fxs_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const usdc_balance_after_redeem = new BigNumber(
			await usdc_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG6);
		...
		const pool_usdc_bal_start = new BigNumber(await usdc_instance.balanceOf.call(pool_instance_V3.address))
			.div(BIG6)
			.toNumber();
		...
		await frax_amo_minter_instance.giveCollatToAMO(curve_amo_v4_instance.address, new BigNumber('300000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'CurveAMO collatDollarBalance:',
			new BigNumber((await curve_amo_v4_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
			'FraxPoolV3 collatDollarBalance:',
			new BigNumber(await pool_instance_V3.collatDollarBalance.call()).div(BIG18).toNumber(),
		);
		console.log(
			'Minter collatDollarBalance:',
			new BigNumber(await frax_amo_minter_instance.collatDollarBalance.call()).div(BIG18).toNumber(),
		);

		console.log(chalk.hex('#ff8b3d').bold('=====================DEPOSIT [USDC ONLY]====================='));
		console.log('Deposit into the metapool');
		const pre_deposit_metapool_usdc_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_amo_v4_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit Curve AMO metapool LP balance:', pre_deposit_metapool_usdc_only);
		const pre_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v4_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit Curve AMO 3pool LP balance:', pre_deposit_3pool_usdc_only);
		console.log(
			'pre-deposit Curve AMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(curve_amo_v4_instance.address)).div(BIG6).toNumber(),
		);
		console.log('');

		console.log('Depositing 10k USDC into the metapool');
		await curve_amo_v4_instance.metapoolDeposit(0, new BigNumber('10000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('');
		const post_deposit_metapool_usdc_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_amo_v4_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('post-deposit Curve AMO metapool LP balance:', post_deposit_metapool_usdc_only);
		const post_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v4_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('post-deposit Curve AMO 3pool LP balance:', post_deposit_3pool_usdc_only);
		console.log(
			'post-deposit Curve AMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(curve_amo_v4_instance.address)).div(BIG6).toNumber(),
		);

		console.log('');
		console.log(
			'metapool LP total supply:',
			new BigNumber(await frax_3crv_metapool_instance.totalSupply()).div(BIG18).toNumber(),
		);
		console.log(
			'metapool 3CRV balance:',
			new BigNumber(await crv3Instance.balanceOf(frax_3crv_metapool_instance.address)).div(BIG18).toNumber(),
		);
		console.log(
			'metapool FRAX balance:',
			new BigNumber(await frax_instance.balanceOf(frax_3crv_metapool_instance.address)).div(BIG18).toNumber(),
		);

		console.log('');
		console.log(
			'Curve AMO dollarBalances [FRAX]:',
			new BigNumber((await curve_amo_v4_instance.dollarBalances())[0]).div(BIG18).toNumber(),
		);
		console.log(
			'Curve AMO dollarBalances [COLLAT]:',
			new BigNumber((await curve_amo_v4_instance.dollarBalances())[1]).div(BIG18).toNumber(),
		);
		...
		console.log('Deposit into the metapool');
		const pre_deposit_metapool_frax_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_amo_v4_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit Curve AMO metapool LP balance:', pre_deposit_metapool_frax_only);
		const pre_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v4_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await frax_amo_minter_instance.mintFraxForAMO(curve_amo_v4_instance.address, new BigNumber('100000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Depositing 100k FRAX into the metapool');
		await curve_amo_v4_instance.metapoolDeposit(new BigNumber('100000e18'), 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('');
		const post_deposit_metapool_frax_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_amo_v4_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('post-deposit Curve AMO metapool LP balance:', post_deposit_metapool_frax_only);
		const post_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(curve_amo_v4_instance.address))
			.div(BIG18)
			.toNumber();
		...
		console.log('Deposit into the metapool');
		const pre_deposit_metapool_both = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_amo_v4_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit Curve AMO metapool LP balance:', pre_deposit_metapool_both);
		const pre_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(curve_amo_v4_instance.address))
			.div(BIG18)
			.toNumber();
		...
		console.log('Get some FRAX from the minter');
		await frax_amo_minter_instance.mintFraxForAMO(curve_amo_v4_instance.address, new BigNumber('5000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Depositing 5k FRAX and 5k USDC into the metapool');
		await curve_amo_v4_instance.metapoolDeposit(new BigNumber('5000e18'), new BigNumber('5000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('');
		const post_deposit_metapool_both = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_amo_v4_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('post-deposit Curve AMO metapool LP balance:', post_deposit_metapool_both);
		const post_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(curve_amo_v4_instance.address))
			.div(BIG18)
			.toNumber();
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=====================WITHDRAW FRAX FROM THE METAPOOL====================='),
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=====================WITHDRAW 3POOL FROM THE METAPOOL====================='),
		);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold(
					'=====================WITHDRAW 3POOL AND USDC FROM THE METAPOOL AT THE SAME TIME=====================',
				),
		);
		...
		await curve_amo_v4_instance.metapoolWithdrawAndConvert3pool(test_amt_3pool_both, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold(
					'=====================WITHDRAW 3POOL AND FRAX FROM THE METAPOOL AT THE CURRENT BALANCE=====================',
				),
		);
		...
		await curve_amo_v4_instance.metapoolWithdrawAtCurRatio(test_amt_frax_usdc, 0, 0, 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const frax_amo_balance = new BigNumber(await frax_instance.balanceOf(frax_3crv_metapool_instance.address))
			.div(BIG18)
			.toNumber();
		const crv3_amo_balance = new BigNumber(await crv3Instance.balanceOf(frax_3crv_metapool_instance.address))
			.div(BIG18)
			.toNumber();
		...
		const amount_in_vault_0 = new BigNumber(await curve_amo_v4_instance.yvCurveFRAXBalance.call())
			.div(BIG18)
			.toNumber();
		const usd_value_in_vault_0 = new BigNumber(await curve_amo_v4_instance.usdValueInVault.call())
			.div(BIG18)
			.toNumber();
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=====================ADVANCE SOME TIME AND SEE GROWTH====================='),
		);
		// Advance a few weeks
		for (let j = 0; j < 10; j++) {
		...
		const usd_value_in_vault_1 = new BigNumber(await curve_amo_v4_instance.usdValueInVault.call())
			.div(BIG18)
			.toNumber();
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=====================WITHDRAW EVERYTHING FROM THE VAULT====================='),
		);
		...
		const fxs_balance_after = new BigNumber(await fxs_instance.balanceOf.call(curve_amo_v4_instance.address)).div(
			BIG18,
		);
		const usdc_balance_after = new BigNumber(await usdc_instance.balanceOf.call(curve_amo_v4_instance.address)).div(
			BIG6,
		);
		const borrowed_balance = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(curve_amo_v4_instance.address),
		).div(BIG6);
		...
		const big_test_collatdollarbalance_before = new BigNumber((await curve_amo_v4_instance.dollarBalances())[1])
			.div(BIG18)
			.toNumber();
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=====================PUT 10k FRAX3CRV IN THE VAULT====================='),
		);
		...
		const amount_in_vault_3 = new BigNumber(await curve_amo_v4_instance.yvCurveFRAXBalance.call())
			.div(BIG18)
			.toNumber();
		const usd_value_in_vault_3 = new BigNumber(await curve_amo_v4_instance.usdValueInVault.call())
			.div(BIG18)
			.toNumber();
		...
		const usd_value_in_vault_4 = new BigNumber(await curve_amo_v4_instance.usdValueInVault.call())
			.div(BIG18)
			.toNumber();
		...
		const fxs_balance_4_after = new BigNumber(await fxs_instance.balanceOf.call(curve_amo_v4_instance.address)).div(
			BIG18,
		);
		const usdc_balance_4_after = new BigNumber(
			await usdc_instance.balanceOf.call(curve_amo_v4_instance.address),
		).div(BIG6);
		const borrowed_balance_4 = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(curve_amo_v4_instance.address),
		).div(BIG6);
		...
		const big_test_collatdollarbalance_after = new BigNumber((await curve_amo_v4_instance.dollarBalances())[1])
			.div(BIG18)
			.toNumber();
		console.log('CurveAMO_V4 dollarBalances after [COLLAT]:', big_test_collatdollarbalance_after);

		console.log(
			'dollarBalances change [COLLAT]:',
			big_test_collatdollarbalance_after - big_test_collatdollarbalance_before,
		);
		...
		await curve_amo_v4_instance.execute(usdc_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const investor_usdc_balance_change = investor_usdc_bal_after
			.minus(investor_usdc_bal_before)
			.div(BIG6)
			.toNumber();
		...
		console.log(
			'Global Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedGlobal.call()).div(BIG18).toNumber(),
		);
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(curve_amo_v4_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
			'fraxDollarBalanceStored:',
			new BigNumber(await frax_amo_minter_instance.fraxDollarBalanceStored.call()).div(BIG18).toNumber(),
		);
		console.log(
			'frax_mint_sum:',
			new BigNumber(await frax_amo_minter_instance.frax_mint_sum.call()).div(BIG18).toNumber(),
		);
		console.log(
			'fxs_mint_sum:',
			new BigNumber(await frax_amo_minter_instance.fxs_mint_sum.call()).div(BIG18).toNumber(),
		);
		console.log(
			'collat_borrowed_sum:',
			new BigNumber(await frax_amo_minter_instance.collat_borrowed_sum.call()).div(BIG6).toNumber(),
		);
		...
		const pool_usdc_bal_end = new BigNumber(await usdc_instance.balanceOf.call(pool_instance_V3.address))
			.div(BIG6)
			.toNumber();
