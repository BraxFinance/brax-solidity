...
contract('UniV3LiquidityAMO_V2 Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(univ3_liquidity_amo_v2_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('====================== TEST Uni v3 NonfungiblePositionManager ====================='),
		);
		...
		console.log(
			...
			new BigNumber(await usdc_instance.balanceOf(univ3_liquidity_amo_v2_instance.address)).div(BIG6).toNumber(),
		);
		console.log(
			'(IF THIS IS ZERO, THIS IS CAUSING THE ERROR) Uni v3 AMO [FRAX balance]:',
			new BigNumber(await frax_instance.balanceOf(univ3_liquidity_amo_v2_instance.address)).div(BIG18).toNumber(),
		);
		...
		console.log(
			...
			new BigNumber(
				await univ3_position_manager_instance.balanceOf(univ3_liquidity_amo_v2_instance.address),
			).toNumber(),
		);
		console.log(
			'AMO numPositions():',
			new BigNumber(await univ3_liquidity_amo_v2_instance.numPositions()).toNumber(),
		);

		const token_id_0 = (
			await univ3_position_manager_instance.tokenOfOwnerByIndex(univ3_liquidity_amo_v2_instance.address, 0)
		).toNumber();
		...
		console.log(
			...
			new BigNumber((await univ3_liquidity_amo_v2_instance.positions_mapping(token_id_0)).liquidity).toNumber(),
		);

		console.log('');
		console.log(
			'UniV3 AMO dollarBalances [FRAX]:',
			new BigNumber((await univ3_liquidity_amo_v2_instance.dollarBalances())[0]).div(BIG18).toNumber(),
		);
		console.log(
			'UniV3 AMO dollarBalances [COLLAT]:',
			new BigNumber((await univ3_liquidity_amo_v2_instance.dollarBalances())[1]).div(BIG18).toNumber(),
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('====================== WITHDRAW LIQUIDITY FROM THE NFT ====================='),
		);
		...
		await univ3_liquidity_amo_v2_instance.swap(
			usdc_instance.address,
			daiInstance.address,
			500,
			new BigNumber('10000e6'),
		...
		console.log(
			...
			new BigNumber(await daiInstance.balanceOf(univ3_liquidity_amo_v2_instance.address)).div(BIG18).toNumber(),
		);
		...
		const token_id_1 = (
			await univ3_position_manager_instance.tokenOfOwnerByIndex(univ3_liquidity_amo_v2_instance.address, 1)
		).toNumber();
		...
		console.log(
			'positions_mapping[token_id].liquidity:',
			new BigNumber((await univ3_liquidity_amo_v2_instance.positions_mapping(token_id_1)).liquidity).toNumber(),
		);
		...
		await univ3_liquidity_amo_v2_instance.recoverERC721(
			CONTRACT_ADDRESSES.ethereum.uniswap_v3.NonfungiblePositionManager,
			token_id_0,
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		);
		...
		await univ3_liquidity_amo_v2_instance.execute(usdc_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await univ3_liquidity_amo_v2_instance.burnFRAX(new BigNumber('1000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Give some USDC BACK');
		await univ3_liquidity_amo_v2_instance.giveCollatBack(new BigNumber('1000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
