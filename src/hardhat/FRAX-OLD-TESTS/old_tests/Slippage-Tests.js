...
contract('FRAX', async (accounts) => {
	...
	it('Check up on the oracles and make sure the prices are set', async () => {
		...
		let USDT_price_from_USDT_WETH = new BigNumber(await oracle_instance_USDT_WETH.consult.call(WETH.address, 1e6))
			.div(1e6)
			.toNumber();
		let USDC_price_from_USDC_WETH = new BigNumber(await oracle_instance_USDC_WETH.consult.call(WETH.address, 1e6))
			.div(1e6)
			.toNumber();
		...
		USDT_price_from_USDT_WETH = new BigNumber(await oracle_instance_USDT_WETH.consult.call(WETH.address, 1e6))
			.div(1e6)
			.toNumber();
		USDC_price_from_USDC_WETH = new BigNumber(await oracle_instance_USDC_WETH.consult.call(WETH.address, 1e6))
			.div(1e6)
			.toNumber();
...
*/
	...
	it('redeemFractionalFRAX() - Testing slippage, FXS amountOut passing && USDC amountOut passing', async () => {
		...
		// Redeem some FRAX
		await pool_instance_USDC.redeemFractionalFRAX(frax_amount, fxs_out, usdc_out, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
	...
	it('mintFractionalFRAX, FRAX amountOut trivially passes', async () => {
		...
		await pool_instance_USDC.mintFractionalFRAX(collateral_amount, fxs_amount, FRAX_out_min, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
	...
	it('mintFractionalFRAX, FRAX amountOut passes', async () => {
		...
		const FRAX_out_min = new BigNumber(
			collateral_amount.times(await pool_instance_USDC.getCollateralPrice()).div(globalCollateralRatio),
		)
			.idiv(BIG6)
			.idiv(BIG6)
			.idiv(BIG6)
		...
		await pool_instance_USDC.mintFractionalFRAX(collateral_amount, fxs_amount, FRAX_out_min_2, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
	...
	it('mintFractionalFRAX, FRAX amountOut fails', async () => {
		...
		await pool_instance_USDC.mintFractionalFRAX(collateral_amount, fxs_amount, FRAX_out_min_3, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
