...
describe('FPI', function () {
	...
	it('stake-wait-unstake', async function () {
		...
		expect(await contracts.fpiStaking.stakingMultiplierAt(lastUpdatedTime + 1 * 24 * 60 * 60)).to.equal(
			'1009950166250831946',
		); // 2-exp(-1/100) = 1.009950166
		expect(await contracts.fpiStaking.stakingMultiplierAt(lastUpdatedTime + 2 * 24 * 60 * 60)).to.equal(
			'1019801326693244697',
		); // 2-exp(-1/2) = 1.019801327
		expect(await contracts.fpiStaking.stakingMultiplierAt(lastUpdatedTime + 10 * 24 * 60 * 60)).to.equal(
			'1095162581964040426',
		); // 2-exp(-1/10) = 1.095162582
		expect(await contracts.fpiStaking.stakingMultiplierAt(lastUpdatedTime + 100 * 24 * 60 * 60)).to.equal(
			'1632120558828557678',
		); // 2-exp(-1) = 1.632120559
