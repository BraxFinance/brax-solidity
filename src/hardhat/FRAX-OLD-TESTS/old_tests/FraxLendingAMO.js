...
contract('FraxLendingAMO-Tests', async (accounts) => {
	...
	it('Initialization related', async () => {
		...
		await frax_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('50000e18'), {
	...
	it('Main test', async () => {
		...
		const frax_balance_before = new BigNumber(await frax_instance.balanceOf.call(lending_amo_instance.address)).div(
			BIG18,
		);
		console.log('FRAX before: ', frax_balance_before.toNumber());

		console.log('Get some FRAX from the minter');
		await frax_amo_minter_instance.mintFraxForAMO(lending_amo_instance.address, new BigNumber('20000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		const frax_balance_after = new BigNumber(await frax_instance.balanceOf.call(lending_amo_instance.address)).div(
			BIG18,
		);
		const borrowed_balance = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(lending_amo_instance.address),
		).div(BIG6);
		...
		const frax_balance_after_burn = new BigNumber(
			await frax_instance.balanceOf.call(lending_amo_instance.address),
		).div(BIG18);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('======================PROVIDE LIQUIDITY FOR CREAM======================'),
		);
		...
		const crFRAX_balance = new BigNumber(await crFRAX_instance.balanceOf.call(lending_amo_instance.address)).div(
			BIG8,
		);
		console.log('crFRAX balance: ', crFRAX_balance.toNumber());

		// Withdraw the crFRAX (will auto convert back to FRAX)
		await lending_amo_instance.creamWithdraw_crFRAX(crFRAX_balance.multipliedBy(BIG8), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const frax_balance_after_withdrawal_cr = new BigNumber(
			await frax_instance.balanceOf.call(lending_amo_instance.address),
		).div(BIG18);
		...
		const frax_balance_before_emergency = new BigNumber(
			await frax_instance.balanceOf.call(lending_amo_instance.address),
		).div(BIG18);
		console.log('FRAX balance before emergency: ', frax_balance_before_emergency.toNumber());
		await lending_amo_instance.recoverERC20(frax_instance.address, new BigNumber('1e15'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const frax_balance_after_emergency = new BigNumber(
			await frax_instance.balanceOf.call(lending_amo_instance.address),
		).div(BIG18);
		console.log('FRAX balance after emergency: ', frax_balance_after_emergency.toNumber());
		console.log(
			'FRAX withdrawn: ',
			frax_balance_before_emergency.toNumber() - frax_balance_after_emergency.toNumber(),
		);

		console.log(chalk.hex('#ff8b3d').bold('=====================NOTE BALANCES====================='));
		const frax_balance_before_giving_back = new BigNumber(
			await frax_instance.balanceOf.call(lending_amo_instance.address),
		).div(BIG18);
		console.log('FRAX balance before giving back: ', frax_balance_before_giving_back.toNumber());
		await lending_amo_instance.burnFRAX(frax_balance_before_giving_back.multipliedBy(BIG18), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const frax_balance_after_giving_back = new BigNumber(
			await frax_instance.balanceOf.call(lending_amo_instance.address),
		).div(BIG18);
		const minted_balance = new BigNumber(
			await frax_amo_minter_instance.mint_balances.call(lending_amo_instance.address),
		).div(BIG18);
		const amo_profit = new BigNumber(
			await frax_amo_minter_instance.fraxTrackedAMO.call(lending_amo_instance.address),
		).div(BIG18);
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(lending_amo_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
