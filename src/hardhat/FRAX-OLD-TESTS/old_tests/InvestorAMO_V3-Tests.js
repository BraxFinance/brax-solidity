...
contract('InvestorAMO_V3-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await fxs_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('100000e18'), {
		...
		const fxs_balance_before = new BigNumber(
			await fxs_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG18);
		const usdc_balance_before = new BigNumber(
			await usdc_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG6);
		...
		await frax_amo_minter_instance.giveCollatToAMO(investor_amo_v3_instance.address, new BigNumber('15000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		const fxs_balance_after = new BigNumber(
			await fxs_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG18);
		...
		const borrowed_balance = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(investor_amo_v3_instance.address),
		).div(BIG6);
		...
		await investor_amo_v3_instance.burnFXS(fxs_balance_after.multipliedBy(BIG18), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		const fxs_balance_after_burn = new BigNumber(
			await fxs_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG18);
		...
		console.log(
			'globalCollateralValue change after pull and test redeem: ',
			global_col_val_after.toNumber() - global_col_val_before.toNumber(),
		);

		console.log(chalk.hex('#ff8b3d').bold('======================INVEST INTO yVault======================'));
		await investor_amo_v3_instance.yDepositUSDC(usdc_balance_after.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const yUSD_balance = new BigNumber(await yUSDC_instance.balanceOf.call(investor_amo_v3_instance.address)).div(
			BIG6,
		);
		console.log('yUSDC balance: ', yUSD_balance.toNumber());
		await investor_amo_v3_instance.yWithdrawUSDC(yUSD_balance.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const usdc_balance_after_withdrawal_y = new BigNumber(
			await usdc_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG6);
		...
		console.log(
			'globalCollateralValue Midpoint Change [includes Rari, etc profits!]:',
			gcv_midpoint_1 - gcv_bal_start,
		);

		// Note the collatDollarBalances
		console.log(
			'InvestorAMO collatDollarBalance:',
			new BigNumber((await investor_amo_v3_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
		...
		await expectRevert.unspecified(
			frax_amo_minter_instance.mintFraxForAMO(investor_amo_v3_instance.address, new BigNumber('1000000000e18'), {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			}),
		);

		console.log(chalk.hex('#ff8b3d').bold('======================INVEST INTO aUSDC======================'));
		await investor_amo_v3_instance.aaveDepositUSDC(usdc_balance_after_withdrawal_y.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const aUSDC_balance = new BigNumber(
			await aUSDC_token_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG6);
		console.log('aUSDC balance: ', aUSDC_balance.toNumber());
		await investor_amo_v3_instance.aaveWithdrawUSDC(aUSDC_balance.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const usdc_balance_after_withdrawal_a = new BigNumber(
			await usdc_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG6);
		...
		await investor_amo_v3_instance.compoundMint_cUSDC(usdc_balance_after_withdrawal_a.multipliedBy(BIG6), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const cUSDC_balance = new BigNumber(await cUSDC_instance.balanceOf.call(investor_amo_v3_instance.address)).div(
			BIG8,
		);
		console.log('cUSDC balance: ', cUSDC_balance.toNumber());
		await investor_amo_v3_instance.compoundRedeem_cUSDC(cUSDC_balance.multipliedBy(BIG8), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const usdc_balance_after_withdrawal_c = new BigNumber(
			await usdc_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG6);
		...
		const usdc_balance_before_recovery = new BigNumber(
			await usdc_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG6);
		console.log('USDC balance before recovery: ', usdc_balance_before_recovery.toNumber());
		await investor_amo_v3_instance.recoverERC20(usdc_instance.address, new BigNumber('1e3'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const usdc_balance_after_recovery = new BigNumber(
			await usdc_instance.balanceOf.call(investor_amo_v3_instance.address),
		).div(BIG6);
		console.log('USDC balance after recovery: ', usdc_balance_after_recovery.toNumber());
		console.log(
			'USDC withdrawn: ',
			usdc_balance_before_recovery.toNumber() - usdc_balance_after_recovery.toNumber(),
		);
		...
		await investor_amo_v3_instance.execute(usdc_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(investor_amo_v3_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
