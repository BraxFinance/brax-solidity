...
contract('Smart Wallet Whitelist Tests', async (accounts) => {
	...
	it('Tests SmartWalletWhitelist', async () => {
		...
		await fxs_instance.transfer(micro_vefxs_staker_instance.address, new BigNumber('1000000e18'), {
		...
		console.log('Commit');
		await veFXS_instance.commit_smart_wallet_checker(smart_wallet_whitelist_instance.address, {
			from: ORIGINAL_FRAX_DEPLOYER_ADDRESS,
		});
		...
		await fxs_instance.approve(veFXS_instance.address, deposit_amount_quick_e18_4_yr, {
		...
		// Try to lock
		await expectRevert(
			micro_vefxs_staker_instance.vefxs_create_lock(deposit_amount_quick_e18_4_yr, deposit_quick_timestamp_4_yr, {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			}),
		...
		await smart_wallet_whitelist_instance.approveWallet(micro_vefxs_staker_instance.address, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		// Try to lock now
		await micro_vefxs_staker_instance.vefxs_create_lock(
			deposit_amount_quick_e18_4_yr,
			deposit_quick_timestamp_4_yr,
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		);
	});
});
