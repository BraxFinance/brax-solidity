...
contract('CommunalFarm-Tests', async (accounts) => {
	...
	beforeEach(async () => {
		...
		pair_instance_Saddle_D4 = await ISaddleD4_LP.at(
			CONTRACT_ADDRESSES.ethereum.pair_tokens['Saddle alUSD/FEI/FRAX/LUSD'],
		);
	...
	it('Initialization related', async () => {
		...
		await frax_instance.transfer(communalFarmInstance_Saddle_D4.address, new BigNumber('1e18'), {
		...
		await fxs_instance.transfer(communalFarmInstance_Saddle_D4.address, new BigNumber('10000e18'), {
		...
		await tribe_instance.transfer(communalFarmInstance_Saddle_D4.address, new BigNumber('10000e18'), {
			from: ADDRESS_WITH_TRIBE,
		});
		...
		await alcx_instance.transfer(communalFarmInstance_Saddle_D4.address, new BigNumber('10000e18'), {
			from: ADDRESS_WITH_ALCX,
		});
		...
		await lqty_instance.transfer(communalFarmInstance_Saddle_D4.address, new BigNumber('10000e18'), {
			from: ADDRESS_WITH_LQTY,
		});
	...
	it('Locked stakes', async () => {
		...
		await utilities.printCalcCurCombinedWeightNoVeFXS(
			communalFarmInstance_Saddle_D4,
		...
		await pair_instance_Saddle_D4.approve(communalFarmInstance_Saddle_D4.address, uni_pool_locked_1_sum, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await pair_instance_Saddle_D4.approve(communalFarmInstance_Saddle_D4.address, uni_pool_locked_9, {
			from: accounts[9],
		});
		...
		// Stake Locked
		// account[1]
		await communalFarmInstance_Saddle_D4.stakeLocked(uni_pool_locked_1, 7 * 86400, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		}); // 15 days
		await communalFarmInstance_Saddle_D4.stakeLocked(new BigNumber('25e17'), 365 * 86400, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		}); // 270 days
		...
		const locked_stake_structs_1_0 = await communalFarmInstance_Saddle_D4.lockedStakesOf.call(
			COLLATERAL_FRAX_AND_FXS_OWNER,
		);
		...
		const regular_balance_1 = new BigNumber(
			await communalFarmInstance_Saddle_D4.lockedLiquidityOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const boosted_balance_1 = new BigNumber(
			await communalFarmInstance_Saddle_D4.combinedWeightOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const locked_balance_1 = new BigNumber(
			await communalFarmInstance_Saddle_D4.lockedLiquidityOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		const regular_balance_9 = new BigNumber(
			await communalFarmInstance_Saddle_D4.lockedLiquidityOf.call(accounts[9]),
		).div(BIG18);
		const boosted_balance_9 = new BigNumber(
			await communalFarmInstance_Saddle_D4.combinedWeightOf.call(accounts[9]),
		).div(BIG18);
		...
		await expectRevert.unspecified(
			communalFarmInstance_Saddle_D4.withdrawLocked(locked_stake_structs_1_0[0].kek_id, {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			}),
		);
		await expectRevert.unspecified(
			communalFarmInstance_Saddle_D4.withdrawLocked(locked_stake_structs_9_0[0].kek_id, { from: accounts[9] }),
		);
		await time.advanceBlock();
		...
		const _total_liquidity_locked_0 = new BigNumber(
			await communalFarmInstance_Saddle_D4.totalLiquidityLocked.call(),
		).div(BIG18);
		const _total_combined_weight_0 = new BigNumber(
			await communalFarmInstance_Saddle_D4.totalCombinedWeight.call(),
		).div(BIG18);
		...
		for (let j = 0; j < staking_reward_symbols.length; j++) {
			...
			console.log(
				`accounts[1] earnings after 1 week [${staking_reward_symbols[j]}]: `,
				staking_earned_1.toString(),
			);
			const staking_earned_9 = new BigNumber(staking_earned_9_arr[j]).div(BIG18);
			console.log(
				`accounts[9] earnings after 1 week [${staking_reward_symbols[j]}]: `,
			...
			console.log(
				`Effective weekly reward at week 1 [${staking_reward_symbols[j]}]: `,
				reward_week_1[j].toString(),
			);
			console.log(
				`Effective yearly reward at week 1 [${staking_reward_symbols[j]}]: `,
				effective_yearly_reward_at_week_1.toString(),
			);

			const duration_reward_1 = new BigNumber(duration_reward_1_arr[j]).div(BIG18);
			console.log(
				`Expected yearly reward [${staking_reward_symbols[j]}]: `,
		...
		await communalFarmInstance_Saddle_D4.withdrawLocked(locked_stake_structs_1_0[0].kek_id, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		await expectRevert.unspecified(
			communalFarmInstance_Saddle_D4.withdrawLocked(locked_stake_structs_9_0[0].kek_id, { from: accounts[9] }),
		...
		const staking_earned_week5_1_arr = await communalFarmInstance_Saddle_D4.earned.call(
			COLLATERAL_FRAX_AND_FXS_OWNER,
		);
		...
		for (let j = 0; j < staking_reward_symbols.length; j++) {
			...
			console.log(
				`accounts[1] earnings after 5 weeks [${staking_reward_symbols[j]}]: `,
				staking_earned_week5_1.toString(),
			);
			const staking_earned_week5_9 = new BigNumber(staking_earned_week5_9_arr[j]).div(BIG18);
			console.log(
				`accounts[9] earnings after 5 weeks [${staking_reward_symbols[j]}]: `,
			...
			console.log(
				`Effective weekly reward after 5 weeks [${staking_reward_symbols[j]}]: `,
				reward_week_5[j].div(4).toString(),
			);
			console.log(
				`Effective yearly reward after 5 weeks [${staking_reward_symbols[j]}]: `,
		...
		await communalFarmInstance_Saddle_D4.getReward({ from: accounts[9] });
		await expectRevert.unspecified(
			communalFarmInstance_Saddle_D4.withdrawLocked(locked_stake_structs_1_0[1].kek_id, {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			}),
		);
		...
		await communalFarmInstance_Saddle_D4.withdrawLocked(locked_stake_structs_1_0[1].kek_id, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		// Approve
		await pair_instance_Saddle_D4.approve(
			communalFarmInstance_Saddle_D4.address,
			uni_pool_lock_boundary_check_amount,
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		);
		await pair_instance_Saddle_D4.approve(
			communalFarmInstance_Saddle_D4.address,
		...
		// Stake Locked
		// account[1]
		await communalFarmInstance_Saddle_D4.stakeLocked(uni_pool_lock_boundary_check_amount, 10 * 86400, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
	...
	it('Blocks a greylisted address which tries to stake; SHOULD FAIL', async () => {
		...
		await pair_instance_Saddle_D4.approve(communalFarmInstance_Saddle_D4.address, new BigNumber('1e18'), {
			from: accounts[9],
		});
	...
	it('Communal / Token Manager Tests', async () => {
		...
		await communalFarmInstance_Saddle_D4.recoverERC20(frax_instance.address, test_recovery_amount, {
			from: STAKING_OWNER,
		});

		console.log('Set a reward rate as the owner');
		await communalFarmInstance_Saddle_D4.setRewardRate(staking_reward_tokens_addresses[3], 1000, true, {
			from: STAKING_OWNER,
		});

		for (let j = 0; j < staking_reward_symbols.length; j++) {
			const token_manager_address = await communalFarmInstance_Saddle_D4.rewardManagers.call(
			...
			await expectRevert(
				communalFarmInstance_Saddle_D4.setRewardRate(staking_reward_tokens_addresses[0], 0, true, {
					from: accounts[9],
				}),
			...
			await expectRevert(
				communalFarmInstance_Saddle_D4.changeTokenManager(
					staking_reward_tokens_addresses[0],
					COLLATERAL_FRAX_AND_FXS_OWNER,
					{ from: accounts[9] },
				),
				'Not owner or tkn mgr',
			...
			await communalFarmInstance_Saddle_D4.setRewardRate(staking_reward_tokens_addresses[j], 0, true, {
				from: token_manager_address,
			});

			console.log('Try recovering reward tokens as the reward manager');
			await communalFarmInstance_Saddle_D4.recoverERC20(
				staking_reward_tokens_addresses[j],
				test_recovery_amount,
				{ from: token_manager_address },
			);

			console.log('Change the token manager');
			await communalFarmInstance_Saddle_D4.changeTokenManager(
				staking_reward_tokens_addresses[j],
				COLLATERAL_FRAX_AND_FXS_OWNER,
				{ from: token_manager_address },
			);
	...
	it('Fail Tests ', async () => {
		...
		const locked_stake_structs = await communalFarmInstance_Saddle_D4.lockedStakesOf.call(
			COLLATERAL_FRAX_AND_FXS_OWNER,
		);

		console.log('---------TRY TO ERC20 RECOVER WHILE NOT AN OWNER [SHOULD FAIL]---------');
		await expectRevert(
			communalFarmInstance_Saddle_D4.recoverERC20(pair_instance_Saddle_D4.address, test_amount_1, {
