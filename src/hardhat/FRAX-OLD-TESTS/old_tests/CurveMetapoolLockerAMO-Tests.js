...
contract('CurveMetapoolLockerAMO-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await usdc_real_instance.transfer(pool_instance_V3.address, new BigNumber('10000000e6'), {
			from: ADDRESS_WITH_USDC,
		});
		...
		const new_method_before = new BigNumber(
			await usdc_instance.balanceOf.call(curve_metapool_locker_instance.address),
		)
			.div(BIG6)
			.toNumber();
		// New method (FraxPoolV3)
		await frax_amo_minter_instance.giveCollatToAMO(
			curve_metapool_locker_instance.address,
			new BigNumber('300000e6'),
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		);
		const new_method_after = new BigNumber(
			await usdc_instance.balanceOf.call(curve_metapool_locker_instance.address),
		)
			.div(BIG6)
			.toNumber();
		...
		await frax_amo_minter_instance.oldPoolRedeem(new BigNumber('10000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const old_method_before = new BigNumber(
			await usdc_instance.balanceOf.call(curve_metapool_locker_instance.address),
		)
			.div(BIG6)
			.toNumber();
		await frax_amo_minter_instance.oldPoolCollectAndGive(curve_metapool_locker_instance.address, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		const old_method_after = new BigNumber(
			await usdc_instance.balanceOf.call(curve_metapool_locker_instance.address),
		)
			.div(BIG6)
			.toNumber();
		...
		console.log(
			'CurveMetapoolLockerAMO collatDollarBalance:',
			new BigNumber((await curve_metapool_locker_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
		...
		const pre_deposit_metapool_usdc_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(curve_metapool_locker_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit CurveMetapoolLockerAMO metapool LP balance:', pre_deposit_metapool_usdc_only);
		const pre_deposit_3pool_usdc_only = new BigNumber(
			await crv3Instance.balanceOf(curve_metapool_locker_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit CurveMetapoolLockerAMO 3pool LP balance:', pre_deposit_3pool_usdc_only);
		console.log(
			'pre-deposit CurveMetapoolLockerAMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(curve_metapool_locker_instance.address)).div(BIG6).toNumber(),
		);
		console.log('');

		console.log('Depositing 10k USDC into the metapool');
		await curve_metapool_locker_instance.metapoolDeposit(new BigNumber('10000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_usdc_only = new BigNumber(
			await crv3Instance.balanceOf(curve_metapool_locker_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		console.log(
			'post-deposit CurveMetapoolLockerAMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(curve_metapool_locker_instance.address)).div(BIG6).toNumber(),
		);
		...
		console.log('');
		console.log(
			'CurveMetapoolLockerAMO dollarBalances [FRAX]:',
			new BigNumber((await curve_metapool_locker_instance.dollarBalances())[0]).div(BIG18).toNumber(),
		);
		console.log(
			'CurveMetapoolLockerAMO dollarBalances [COLLAT]:',
			new BigNumber((await curve_metapool_locker_instance.dollarBalances())[1]).div(BIG18).toNumber(),
		);
		...
		await curve_metapool_locker_instance.metapoolWithdraw3pool(test_amt_3pool, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_metapool_locker_instance.three_pool_to_collateral(bal_3pool, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_metapool_locker_instance.metapoolWithdrawAndConvert3pool(test_amt_3pool_both, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_metapool_locker_instance.depositToVault(deposit_amount_e18, 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const amount_in_vault_0 = new BigNumber(await curve_metapool_locker_instance.stakedBalance.call())
			.div(BIG18)
			.toNumber();
		const usd_value_in_vault_0 = new BigNumber(await curve_metapool_locker_instance.usdValueInVault.call())
			.div(BIG18)
			.toNumber();
		...
		const usd_value_in_vault_1 = new BigNumber(await curve_metapool_locker_instance.usdValueInVault.call())
			.div(BIG18)
			.toNumber();
		...
		const tribe_pool_bal_before = new BigNumber(
			await tribe_instance.balanceOf.call(curve_metapool_locker_instance.address),
		).div(BIG18);
		await curve_metapool_locker_instance.harvest({ from: COLLATERAL_FRAX_AND_FXS_OWNER });
		const tribe_pool_bal_after = new BigNumber(
			await tribe_instance.balanceOf.call(curve_metapool_locker_instance.address),
		).div(BIG18);
		console.log('TRIBE CHANGE [pool]:', tribe_pool_bal_after - tribe_pool_bal_before);

		console.log(chalk.hex('#ff8b3d').bold('===================== WITHDRAW TRIBE ====================='));
		const tribe_custodian_bal_before = new BigNumber(
			await tribe_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		await curve_metapool_locker_instance.withdrawRewards({ from: COLLATERAL_FRAX_AND_FXS_OWNER });
		const tribe_custodian_bal_after = new BigNumber(
			await tribe_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		...
		const fxs_balance_4_after = new BigNumber(
			await fxs_instance.balanceOf.call(curve_metapool_locker_instance.address),
		).div(BIG18);
		const usdc_balance_4_after = new BigNumber(
			await usdc_instance.balanceOf.call(curve_metapool_locker_instance.address),
		).div(BIG6);
		const borrowed_balance_4 = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(curve_metapool_locker_instance.address),
		).div(BIG6);
		...
		const big_test_collatdollarbalance_after = new BigNumber(
			(await curve_metapool_locker_instance.dollarBalances())[1],
		)
			.div(BIG18)
			.toNumber();
		...
		await curve_metapool_locker_instance.execute(usdc_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(curve_metapool_locker_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
