...
contract('FXS1559AMO-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		console.log(
			'FXS1559 AMO collatDollarBalance:',
			new BigNumber((await fxs1559_amo_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=========================RUN A MANUAL SWAP BURN========================='),
		);
		...
		await frax_amo_minter_instance.mintFraxForAMO(fxs1559_amo_instance.address, new BigNumber('5000000e18'), {
		...
		const yield_distributor_FXS_before = new BigNumber(
			await fxs_instance.balanceOf(veFXSYieldDistributor_instance.address),
		)
			.div(BIG18)
			.toNumber();
		...
		const frax_balance_before_emergency = new BigNumber(
			await frax_instance.balanceOf.call(fxs1559_amo_instance.address),
		).div(BIG18);
		console.log('FRAX balance before emergency: ', frax_balance_before_emergency.toNumber());
		await fxs1559_amo_instance.recoverERC20(frax_instance.address, new BigNumber('1e15'), {
