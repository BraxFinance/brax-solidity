...
describe('FPI Staking2', function () {
	...
	it('stake', async function () {
		...
		expect(await contracts.fpiToken.balanceOf(contracts.fpiStaking.address)).to.equal(
			(stakeAmount + rewardsAmount).toString(),
		);
		expect(await contracts.fpiToken.balanceOf(owner.address)).to.equal(balanceStart - rewardsAmount);

		// sFPI tokens have been minted
		expect(await contracts.sFPIToken.totalSupply()).to.be.closeTo(
			stakeAmount.toString(),
			ethers.utils.parseUnits('100000000000000', 'wei'),
		);
		expect(await contracts.sFPIToken.balanceOf(owner.address)).to.be.closeTo(
			stakeAmount.toString(),
			ethers.utils.parseUnits('100000000000000', 'wei'),
		);
	});
	...
	it('stakingMultiplierAt', async function () {
		...
		expect(await contracts.fpiStaking.stakingMultiplierAt(contractsInitTime + 0)).to.equal(
			'1000000000000000000',
			precision,
		); // 1
		expect(await contracts.fpiStaking.stakingMultiplierAt(contractsInitTime + 1 * 24 * 60 * 60)).to.be.closeTo(
			'1010000000000000000',
			precision,
		); // 1.01^1 = 1.01
		expect(await contracts.fpiStaking.stakingMultiplierAt(contractsInitTime + 2 * 24 * 60 * 60)).to.be.closeTo(
			'1020100000000000000',
			precision,
		); // 1.01^2 = 1.0201
		expect(await contracts.fpiStaking.stakingMultiplierAt(contractsInitTime + 10 * 24 * 60 * 60)).to.be.closeTo(
			'1104622125411200000',
			precision,
		); // 1.01^10 = 1.104622125
		expect(await contracts.fpiStaking.stakingMultiplierAt(contractsInitTime + 100 * 24 * 60 * 60)).to.be.closeTo(
			'2704813829421530000',
			precision,
	...
	it('stake-wait-unstake', async function () {
		...
		var expectedBalanceEnd =
			BigInt(balanceStart) +
			(BigInt('2704813829421530000') * BigInt(sFPITokenAmount)) / BigInt('1000000000000000000');
		expect(await contracts.fpiToken.balanceOf(owner.address)).to.be.closeTo(
			expectedBalanceEnd.toString(),
			'10000000000000000',
		);
