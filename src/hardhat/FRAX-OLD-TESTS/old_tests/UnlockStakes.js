...
const NonfungiblePositionManager = artifacts.require(
	'Uniswap_V3/uniswap-v3-periphery/interfaces/INonfungiblePositionManager',
);
...
contract('Contract & Environment Tests', async (accounts) => {
	...
	beforeEach(async () => {
		...
		FraxUniV3Farm_Stable_FRAX_USDC_instance = await FraxUniV3Farm_Stable_FRAX_USDC.at(
			'0x1C21Dd0cE3bA89375Fc39F1B134AD15671022660',
		);
	...
	it("withdraws a user's locked FRAX-USDC stake and deposits it into uni v3", async () => {
		...
		await univ2_frax_usdc_instance.approve(routerInstance.address, new BigNumber('100e18'), {
			from: locked_staker,
		});
		...
		console.log(
			'locked_staker FRAX balance:',
			new BigNumber(await frax_instance.balanceOf(locked_staker)).div(BIG18).toNumber(),
		);
		console.log(
			'locked_staker USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(locked_staker)).div(BIG6).toNumber(),
		);
	...
	it('stakes the uni v3 NFT locked', async () => {
		...
		await nft_manager_instance.approve(FraxUniV3Farm_Stable_FRAX_USDC_instance.address, nft_id, {
			from: locked_staker,
			gasLimit: new BigNumber('1e6'),
		});
		await FraxUniV3Farm_Stable_FRAX_USDC_instance.stakeLocked(nft_id, 86400 * 3, {
			from: locked_staker,
			gasLimit: new BigNumber('1e6'),
		});
	});
	...
	it('pays the miner', async () => {
		...
		console.log('checkParentHash:', await migrationBundleUtils_instance.checkParentHash(latestBlock.parentHash));
		await migrationBundleUtils_instance.payFlashbotsMiner(
			latestBlock.hash,
			new BigNumber('1e16'),
			locked_staker,
			nft_id,
			{ from: accounts[0], value: new BigNumber('1e16'), gasLimit: new BigNumber('1e6') },
		); // 0.01 ETH
	});
