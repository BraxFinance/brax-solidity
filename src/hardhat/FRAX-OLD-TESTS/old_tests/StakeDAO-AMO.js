...
contract('StakeDAO AMO Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await crv3Instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber('10100e18'), {
			from: ADDRESS_WITH_3CRV,
		});
		...
		await stakedao_controller_instance.approveStrategy(
			'0xd632f22692fac7611d2aa1c0d552930d43caed3b',
			STAKE_DAO_FRAX_STRATEGY,
			{ from: STAKE_DAO_STRATEGIST },
		);
		await stakedao_controller_instance.setStrategy(
			'0xd632f22692fac7611d2aa1c0d552930d43caed3b',
			STAKE_DAO_FRAX_STRATEGY,
			{ from: STAKE_DAO_STRATEGIST },
		);
		...
		await frax_amo_minter_instance.giveCollatToAMO(stakedao_amo_instance.address, new BigNumber('300000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'StakeDAO AMO collatDollarBalance:',
			new BigNumber((await stakedao_amo_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
		...
		const pre_deposit_metapool_usdc_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(stakedao_amo_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit StakeDAO AMO metapool LP balance:', pre_deposit_metapool_usdc_only);
		const pre_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(stakedao_amo_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit StakeDAO AMO 3pool LP balance:', pre_deposit_3pool_usdc_only);
		console.log(
			'pre-deposit StakeDAO AMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(stakedao_amo_instance.address)).div(BIG6).toNumber(),
		);
		console.log('');

		console.log('Depositing 10k USDC into the metapool');
		await stakedao_amo_instance.metapoolDeposit(0, new BigNumber('10000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(stakedao_amo_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('post-deposit StakeDAO AMO 3pool LP balance:', post_deposit_3pool_usdc_only);
		console.log(
			'post-deposit StakeDAO AMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(stakedao_amo_instance.address)).div(BIG6).toNumber(),
		);
		...
		console.log('');
		console.log(
			'StakeDAO AMO dollarBalances [FRAX]:',
			new BigNumber((await stakedao_amo_instance.dollarBalances())[0]).div(BIG18).toNumber(),
		);
		console.log(
			'StakeDAO AMO dollarBalances [COLLAT]:',
			new BigNumber((await stakedao_amo_instance.dollarBalances())[1]).div(BIG18).toNumber(),
		);
		...
		const pre_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(stakedao_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await frax_amo_minter_instance.mintFraxForAMO(stakedao_amo_instance.address, new BigNumber('100000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Depositing 100k FRAX into the metapool');
		await stakedao_amo_instance.metapoolDeposit(new BigNumber('100000e18'), 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(stakedao_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		const pre_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(stakedao_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		console.log('Get some FRAX from the minter');
		await frax_amo_minter_instance.mintFraxForAMO(stakedao_amo_instance.address, new BigNumber('5000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Depositing 5k FRAX and 5k USDC into the metapool');
		await stakedao_amo_instance.metapoolDeposit(new BigNumber('5000e18'), new BigNumber('5000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(stakedao_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await stakedao_amo_instance.metapoolWithdrawAndConvert3pool(test_amt_3pool_both, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await stakedao_amo_instance.metapoolWithdrawAtCurRatio(test_amt_frax_usdc, 0, 0, 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const frax3crv_in_vault_0 = new BigNumber(await stakedao_amo_instance.FRAX3CRVInVault.call())
			.div(BIG18)
			.toNumber();
		...
		const frax3crv_in_vault_1 = new BigNumber(await stakedao_amo_instance.FRAX3CRVInVault.call())
			.div(BIG18)
			.toNumber();
		...
		const fxs_balance_after = new BigNumber(await fxs_instance.balanceOf.call(stakedao_amo_instance.address)).div(
			BIG18,
		);
		const usdc_balance_after = new BigNumber(await usdc_instance.balanceOf.call(stakedao_amo_instance.address)).div(
			BIG6,
		);
		const borrowed_balance = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(stakedao_amo_instance.address),
		).div(BIG6);
		...
		await stakedao_amo_instance.execute(usdc_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(stakedao_amo_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
