...
contract('TokenTrackerV2-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		let investor_custodian_token_addrs = await token_tracker_v2.allTokensForAddress.call(
			MAINNET_INVESTOR_CUSTODIAN_ADDRESS,
		);
		let missing_dec_test_wallet_addrs = await token_tracker_v2.allTokensForAddress.call(
			MISSING_DEC_TEST_WALLET_ADDR,
		);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('============CHECK BALANCES FOR DIFFERENT ADDRESSES [ETH + TOKENS] (FULL VALUE)============'),
		);
		...
		for (let i = 0; i < comptroller_tkn_addresses.length; i++) {
			let the_value_BN = await token_tracker_v2.getTokenValueInAddress.call(
				COMPTROLLER_ADDRESS,
				comptroller_tkn_addresses[i],
				false,
			);
		...
		let investor_custodian_tkn_addresses = await token_tracker_v2.allTokensForAddress.call(
			MAINNET_INVESTOR_CUSTODIAN_ADDRESS,
		);
		for (let i = 0; i < investor_custodian_tkn_addresses.length; i++) {
			let the_value_BN = await token_tracker_v2.getTokenValueInAddress.call(
				MAINNET_INVESTOR_CUSTODIAN_ADDRESS,
				investor_custodian_tkn_addresses[i],
				false,
			);
			let the_value = new BigNumber(the_value_BN).div(BIG18).toString();
		...
		let comptroller_value = new BigNumber(
			await token_tracker_v2.getValueInAddress.call(COMPTROLLER_ADDRESS, false),
		).div(BIG18);
		let investor_custodian_value = new BigNumber(
			await token_tracker_v2.getValueInAddress.call(MAINNET_INVESTOR_CUSTODIAN_ADDRESS, false),
		).div(BIG18);
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('============CHECK BALANCES FOR DIFFERENT ADDRESSES [ETH + TOKENS] (CR VALUE)============'),
		);
		...
		// Investor Custodian
		console.log(chalk.hex('#34baeb')('---------- Investor Custodian ----------'));
		investor_custodian_tkn_addresses = await token_tracker_v2.allTokensForAddress.call(
			MAINNET_INVESTOR_CUSTODIAN_ADDRESS,
		);
		for (let i = 0; i < investor_custodian_tkn_addresses.length; i++) {
			let the_value_BN = await token_tracker_v2.getTokenValueInAddress.call(
		...
		// Totals
		console.log(chalk.hex('#34baeb')('---------- Totals ----------'));
		comptroller_value = new BigNumber(await token_tracker_v2.getValueInAddress.call(COMPTROLLER_ADDRESS, true)).div(
			BIG18,
		);
		investor_custodian_value = new BigNumber(
			await token_tracker_v2.getValueInAddress.call(MAINNET_INVESTOR_CUSTODIAN_ADDRESS, true),
		).div(BIG18);
