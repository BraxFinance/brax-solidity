...
contract('Convex AMO Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		console.log(
			'Convex AMO collatDollarBalance:',
			new BigNumber((await convex_amo_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(convex_amo_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
		...
		await frax_amo_minter_instance.giveCollatToAMO(convex_amo_instance.address, new BigNumber('300000e6'), {
		...
		const pre_deposit_metapool_usdc_only = new BigNumber(
			await frax_3crv_metapool_instance.balanceOf(convex_amo_instance.address),
		)
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit Convex AMO metapool LP balance:', pre_deposit_metapool_usdc_only);
		const pre_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(convex_amo_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('pre-deposit Convex AMO 3pool LP balance:', pre_deposit_3pool_usdc_only);
		console.log(
			'pre-deposit Convex AMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(convex_amo_instance.address)).div(BIG6).toNumber(),
		);
		console.log('');
		...
		const post_deposit_3pool_usdc_only = new BigNumber(await crv3Instance.balanceOf(convex_amo_instance.address))
			.div(BIG18)
			.toNumber();
		console.log('post-deposit Convex AMO 3pool LP balance:', post_deposit_3pool_usdc_only);
		console.log(
			'post-deposit Convex AMO USDC balance:',
			new BigNumber(await usdc_instance.balanceOf(convex_amo_instance.address)).div(BIG6).toNumber(),
		);
		...
		console.log(
			'Convex AMO dollarBalances [FRAX]:',
			new BigNumber((await convex_amo_instance.dollarBalances())[0]).div(BIG18).toNumber(),
		);
		console.log(
			'Convex AMO dollarBalances [COLLAT]:',
			new BigNumber((await convex_amo_instance.dollarBalances())[1]).div(BIG18).toNumber(),
		);
		...
		const pre_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(convex_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await frax_amo_minter_instance.mintFraxForAMO(convex_amo_instance.address, new BigNumber('100000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Depositing 100k FRAX into the metapool');
		await convex_amo_instance.metapoolDeposit(new BigNumber('100000e18'), 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_frax_only = new BigNumber(await crv3Instance.balanceOf(convex_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		const pre_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(convex_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		console.log('Get some FRAX from the minter');
		await frax_amo_minter_instance.mintFraxForAMO(convex_amo_instance.address, new BigNumber('5000e18'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		console.log('Depositing 5k FRAX and 5k USDC into the metapool');
		await convex_amo_instance.metapoolDeposit(new BigNumber('5000e18'), new BigNumber('5000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const post_deposit_3pool_both = new BigNumber(await crv3Instance.balanceOf(convex_amo_instance.address))
			.div(BIG18)
			.toNumber();
		...
		await convex_amo_instance.metapoolWithdrawAndConvert3pool(test_amt_3pool_both, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const fxs_balance_before_staking = new BigNumber(
			await fxs_instance.balanceOf.call(convex_amo_instance.address),
		);
		...
		const frax3crv_in_vault_0 = new BigNumber(await convex_amo_instance.FRAX3CRVInVault.call())
			.div(BIG18)
			.toNumber();
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('=====================CLAIM REWARDS FROM FRAX3CRV LP STAKING====================='),
		);
		...
		await convex_amo_instance.withdrawAndUnwrapFRAX3CRV(withdrawal_amount_e18, true, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const fxs_balance_after = new BigNumber(await fxs_instance.balanceOf.call(convex_amo_instance.address)).div(
			BIG18,
		);
		const usdc_balance_after = new BigNumber(await usdc_instance.balanceOf.call(convex_amo_instance.address)).div(
			BIG6,
		);
		const borrowed_balance = new BigNumber(
			await frax_amo_minter_instance.collat_borrowed_balances.call(convex_amo_instance.address),
		).div(BIG6);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('=====================COLLECT REWARDS (TO CUSTODIAN)====================='),
		);
		...
		const cvxCRV_balance_beforehand = new BigNumber(
			await cvxCRV_instance.balanceOf.call(INVESTOR_CUSTODIAN_ADDRESS),
		);
		...
		const cvxCRV_balance_afterwards = new BigNumber(
			await cvxCRV_instance.balanceOf.call(INVESTOR_CUSTODIAN_ADDRESS),
		);
		...
		console.log(
			'CUSTODIAN CRV BALANCE CHANGE: ',
			crv_balance_afterwards.minus(crv_balance_beforehand).div(BIG18).toNumber(),
		);
		console.log(
			'CUSTODIAN CVX BALANCE CHANGE: ',
			cvx_balance_afterwards.minus(cvx_balance_beforehand).div(BIG18).toNumber(),
		);
		console.log(
			'CUSTODIAN cvxCRV BALANCE CHANGE: ',
			cvxCRV_balance_afterwards.minus(cvxCRV_balance_beforehand).div(BIG18).toNumber(),
		);
		console.log(
			'CUSTODIAN FXS BALANCE CHANGE: ',
			fxs_balance_afterwards.minus(fxs_balance_beforehand).div(BIG18).toNumber(),
		);
