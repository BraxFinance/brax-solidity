...
contract('RariFuseLendingAMO_V2 AMO Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		console.log(
			'Rari AMO collatDollarBalance:',
			new BigNumber((await rari_amo_instance.dollarBalances.call())[1]).div(BIG18).toNumber(),
		);
		console.log(
		...
		await frax_amo_minter_instance.mintFraxForAMO(rari_amo_instance.address, new BigNumber('500250e18'), {
		...
		// Enter the market
		await usdcRariUnitroller_instance.enterMarkets([usdcRariDelegator_instance.address], {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});

		await usdc_instance.approve(usdcRariDelegator_instance.address, new BigNumber('100000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		const frax_balance_loaning_before = new BigNumber(
			await frax_instance.balanceOf.call(rari_amo_instance.address),
		).div(BIG18);

		// Loop through the pools
		for (let i = 0; i < pool_addresses.length; i++) {
			// Loan some FRAX
			await rari_amo_instance.lendToPool(pool_addresses[i], new BigNumber('50000e18'), {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			});
			console.log(
				`[${pool_addresses[i]}] balance: `,
				new BigNumber(await rari_amo_instance.fraxInPoolByPoolIdx.call(i)).div(BIG18).toNumber(),
			);
		}

		const frax_balance_loaning_after = new BigNumber(
			await frax_instance.balanceOf.call(rari_amo_instance.address),
		).div(BIG18);
		...
		const frax_balance_borrowing_before = new BigNumber(
			await frax_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		...
		const frax_balance_borrowing_after = new BigNumber(
			await frax_instance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER),
		).div(BIG18);
		...
		const frax_balance_redeeming_before = new BigNumber(
			await frax_instance.balanceOf.call(rari_amo_instance.address),
		).div(BIG18);
		...
		for (let i = 0; i < pool_addresses.length; i++) {
			...
			await rari_amo_instance.redeemFromPool(pool_addresses[i], new BigNumber('50000e18'), {
				from: COLLATERAL_FRAX_AND_FXS_OWNER,
			});
			console.log(
				`[${pool_addresses[i]}] balance: `,
		...
		const frax_balance_redeeming_after = new BigNumber(
			await frax_instance.balanceOf.call(rari_amo_instance.address),
		).div(BIG18);
		...
		console.log(
			'AMO Profit:',
			new BigNumber(await frax_amo_minter_instance.fraxTrackedAMO.call(rari_amo_instance.address))
				.div(BIG18)
				.toNumber(),
		);
		console.log(
	...
	it('Fail Tests ', async () => {
		...
		await expectRevert(
			rari_amo_instance.enterMarkets(
		...
		await expectRevert(
			rari_amo_instance.lendToPool('0x0000000000000000000000000000000000000001', new BigNumber('100e18'), {
				from: accounts[9],
			}),
			'Invalid pool',
		);
		...
		await expectRevert(
			rari_amo_instance.redeemFromPool('0x0000000000000000000000000000000000000001', new BigNumber('100e18'), {
				from: accounts[9],
			}),
			'Invalid pool',
		);
