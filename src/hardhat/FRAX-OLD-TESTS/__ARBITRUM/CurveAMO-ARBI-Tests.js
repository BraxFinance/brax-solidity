...
contract('CurveAMO_ARBI-Tests', async (accounts) => {
	...
	it('Main test', async () => {
		...
		await cc_bridge_backer_instance.nominateNewOwner(COLLATERAL_FRAX_AND_FXS_OWNER, {
			from: ORIGINAL_ARBITRUM_ONE_ADDRESS,
		});
		...
		await arbiUSDC_instance.transfer(cc_bridge_backer_instance.address, new BigNumber('1000e6'), {
		...
		await arbiUSDT_instance.transfer(curve_amo_arbi_instance.address, new BigNumber('1000e6'), {
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold(
					'===========================LEND SOME CANONICAL TOKENS TO THE CurveAMO_ARBI===========================',
				),
		);

		console.log('Lend 1000 canFRAX');
		await cc_bridge_backer_instance.lendFraxToAMO(curve_amo_arbi_instance.address, new BigNumber('1000e18'), {
		...
		await cc_bridge_backer_instance.lendCollatToAMO(curve_amo_arbi_instance.address, new BigNumber('1000e6'), {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('===========================DEPOSIT FRAX ALONE FOR LP TOKENS==========================='),
		);

		console.log('Deposit 50 canFRAX');
		const frax_only_deposit_amt = new BigNumber('50e18');
		await curve_amo_arbi_instance.metapoolDeposit(frax_only_deposit_amt, 0, 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('===========================DEPOSIT USDC ALONE FOR LP TOKENS==========================='),
		);

		console.log('Deposit 50 arbiUSDC');
		const usdc_only_deposit_amt = new BigNumber('50e6');
		await curve_amo_arbi_instance.metapoolDeposit(0, usdc_only_deposit_amt, 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold('===========================DEPOSIT USDT ALONE FOR LP TOKENS==========================='),
		);

		console.log('Deposit 50 arbiUSDT');
		const usdt_only_deposit_amt = new BigNumber('50e6');
		await curve_amo_arbi_instance.metapoolDeposit(0, 0, usdt_only_deposit_amt, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk
				.hex('#ff8b3d')
				.bold(
					'===========================DEPOSIT FRAX, USDC, AND USDT FOR LP TOKENS===========================',
				),
		);
		...
		await curve_amo_arbi_instance.metapoolDeposit(
			frax_mixed_deposit_amt,
			usdc_mixed_deposit_amt,
			usdt_mixed_deposit_amt,
			{ from: COLLATERAL_FRAX_AND_FXS_OWNER },
		);
		...
		console.log(
			chalk.hex('#ff8b3d').bold('===========================WITHDRAW FRAX ONLY==========================='),
		);

		console.log('Withdraw 10 FRAX');
		const frax_only_lp_withdraw_amt = new BigNumber('10e18');
		await curve_amo_arbi_instance.metapoolWithdrawFrax(frax_only_lp_withdraw_amt, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk.hex('#ff8b3d').bold('===========================WITHDRAW USDC ONLY==========================='),
		);

		console.log('Withdraw 10 USDC');
		const usdc_only_lp_withdraw_amt = new BigNumber('10e18');
		await curve_amo_arbi_instance.metapoolWithdrawUsdc(usdc_only_lp_withdraw_amt, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk.hex('#ff8b3d').bold('===========================WITHDRAW USDT ONLY==========================='),
		);

		console.log('Withdraw 10 USDT');
		const usdt_only_lp_withdraw_amt = new BigNumber('10e18');
		await curve_amo_arbi_instance.metapoolWithdrawUsdt(usdt_only_lp_withdraw_amt, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log(
			chalk.hex('#ff8b3d').bold('===========================WITHDRAW MIX OF TOKENS==========================='),
		);

		console.log('Withdraw a mix of tokens using 100 LP');
		const mixed_lp_withdraw_amt = new BigNumber('100e18');
		await curve_amo_arbi_instance.metapoolWithdrawAtCurRatio(mixed_lp_withdraw_amt, 0, 0, 0, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_amo_arbi_instance.giveFRAXBack(new BigNumber('1e18'), false, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_amo_arbi_instance.giveCollatBack(new BigNumber('1e6'), false, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		console.log('Give back 1 canFRAX');
		await curve_amo_arbi_instance.giveFRAXBack(new BigNumber('1e18'), true, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
		...
		await curve_amo_arbi_instance.execute(canFRAX_instance.address, 0, calldata, {
			from: COLLATERAL_FRAX_AND_FXS_OWNER,
		});
