...
const { bigNumberify, expandTo18Decimals, encodePrice, getCreate2Address, getApprovalDigest } = require('./utilities');
...
describe('UniswapV2 Tests', function () {
	...
	describe('Core Tests - UniswapV2ERC20', function () {
		...
		beforeEach(async function () {
			...
			const TOKEN = await ethers.getContractFactory(
				'contracts/Uniswap_V2_TWAMM/core/test/ERC20CoreTest.sol:ERC20CoreTest',
			);
	...
	describe('Periphery Tests - UniV2TWAMMRouter', function () {
		async function addLiquidity(router, user1, DTT, DTTAmount, WETHAmount) {
			...
			await router
				.connect(user1)
				.addLiquidityETH(DTT.address, DTTAmount, DTTAmount, WETHAmount, user1.address, constants.MaxUint256, {
					value: WETHAmount,
				});
		...
		describe('UniV2TWAMMRouter', function () {
			...
			it('getAmountOut', async () => {
				expect(await router.getAmountOut(bigNumberify(2), bigNumberify(100), bigNumberify(100))).to.eq(
					bigNumberify(1),
				);
				await expect(
					router.getAmountOut(bigNumberify(0), bigNumberify(100), bigNumberify(100)),
				).to.be.revertedWith('UniV2TWAMMLibrary: INSUFFICIENT_INPUT_AMOUNT');
				await expect(
					router.getAmountOut(bigNumberify(2), bigNumberify(0), bigNumberify(100)),
				).to.be.revertedWith('UniV2TWAMMLibrary: INSUFFICIENT_LIQUIDITY');
				await expect(
			...
			it('getAmountIn', async () => {
				expect(await router.getAmountIn(bigNumberify(1), bigNumberify(100), bigNumberify(100))).to.eq(
					bigNumberify(2),
				);
				await expect(
					router.getAmountIn(bigNumberify(0), bigNumberify(100), bigNumberify(100)),
				).to.be.revertedWith('UniV2TWAMMLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
				await expect(
