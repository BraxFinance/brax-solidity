...
const allTwammTests = (multiplier) =>
	describe(`TWAMM - swap multiplier: ${multiplier} longterm swap ratio: ${outputRatio(
		expandTo18Decimals(10 * multiplier),
		...
		describe('Whitelist', function () {
			...
			it('Everyone can call executeVirtualOrders', async function () {
				...
				await expect(twamm.connect(addr1).executeVirtualOrders((await getBlockTimestamp()) + 1)).to.not.be
					.reverted; // anyone can call this function
			});
		...
		describe('TWAMM Functionality ', function () {
			...
			describe('Long term swaps', function () {
				it('Single sided long term order behaves like normal swap', async function () {
					...
					const expectedOut = token1Reserve.mul(amountIn0MinusFee).div(token0Reserve.add(amountIn0MinusFee));
				...
				it('Orders in both pools work as expected', async function () {
					...
					//including the fee
					expect(amountToken0Bought.add(amountToken1Bought).div(2)).to.be.closeTo(
						amountIn,
						amountIn.mul(998).div(1000),
					);
				});
				...
				it('Swap amounts are consistent with twamm formula', async function () {
					...
					const [finalAReserveExpectedBN, finalBReserveExpectedBN, token0OutBN, token1OutBN] =
					...
					expect(owner_token0_diff.add(owner_token1_diff).div(BIG18).toNumber()).to.be.closeTo(
						lp_diff_value,
						lp_diff_value / 100,
					);
					...
					expect(
						finalAReserveActualBN,
						outputErrorDiffPct(finalAReserveActualBN, finalAReserveExpectedBN),
					).to.be.closeTo(finalAReserveExpectedBN, finalAReserveExpectedBN.div(99));
					expect(
						finalBReserveActualBN,
						outputErrorDiffPct(finalBReserveActualBN, finalBReserveExpectedBN),
					).to.be.closeTo(finalBReserveExpectedBN, finalBReserveExpectedBN.div(99));

					expect(amountToken0Bought, outputErrorDiffPct(amountToken0Bought, token0OutBN)).to.be.closeTo(
						token0OutBN,
						token0OutBN.div(80),
					);
					expect(amountToken1Bought, outputErrorDiffPct(amountToken1Bought, token1OutBN)).to.be.closeTo(
						token1OutBN,
						token1OutBN.div(80),
					);
				});
				...
				it('Multiple orders in both pools work as expected [normal]', async function () {
					...
					expect(
						amountToken0Bought.add(amountToken1Bought).div(2),
						outputErrorDiffPct(
							amountToken0Bought.add(amountToken1Bought).div(2),
							amountIn.mul(997).div(1000),
						),
				...
				it('Normal swap works as expected while long term orders are active', async function () {
					...
					expect(
						amountToken0Bought,
						outputErrorDiffPct(amountToken0Bought, amountToken1Bought),
					).to.be.closeTo(amountToken1Bought, amountIn.div(80));
				});
			});
		...
		async function setupContracts(createPair = true) {
			...
			const DummyToken = await ethers.getContractFactory(
				'contracts/Uniswap_V2_TWAMM/periphery/test/ERC20PeriTest.sol:ERC20PeriTest',
			);
