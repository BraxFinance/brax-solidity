...
async function getAmountOutWithTwamm(amountIn, pair, isToken1 = false) {
	...
	const [token0Rate, token1Rate, lastVirtualOrderBlock, orderBlockInterval, rewardFactorPool0, rewardFactorPool1] =
		await pair.getTwammState();

	const [reserve0, reserve1, lastAmmUpdate, twammReserve0, twammReserve1] = await pair.getTwammReserves();
...
function calculateTwammExpected(tokenAIn, tokenBIn, tokenAReserveRet, tokenBReserveRet) {
	...
	const c = sqrt(tokenAReserve.mul(tokenBInWithFee))
		.sub(sqrt(tokenBReserve.mul(tokenAInWithFee)))
		.div(sqrt(tokenAReserve.mul(tokenBInWithFee)).add(sqrt(tokenBReserve.mul(tokenAInWithFee))));
	//     (
	...
	const finalAReserveExpected = sqrt(k.mul(tokenAInWithFee).div(tokenBInWithFee))
		.mul(exp(exponent).add(c))
		.div(exp(exponent).sub(c));
	//     (
	...
	const finalAReserveExpectedBN = bigNumberify(
		bigmath.format(bigmath.round(finalAReserveExpected), { notation: 'fixed' }),
	);
	const finalBReserveExpectedBN = bigNumberify(
		bigmath.format(bigmath.round(finalBReserveExpected), { notation: 'fixed' }),
	);
