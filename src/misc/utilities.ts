...
export const printAllocations = (key: string, allocations: number[], old_allocations?: number[]) => {
	...
	allocations.forEach((val, idx) => {
		...
		if (the_alloc) {
			...
			console.log(
				`${the_alloc.title}: `,
				chalk.yellow(`${new_value.toPrecision(9)} ${the_alloc.symbol}${change_string}`),
			);
		}
	});
...
export const printTokenBalances = (key: string, tkn_bals: number[], old_tkn_bals?: number[]) => {
	...
	tkn_bals.forEach((val, idx) => {
		...
		if (the_bal) {
			...
			console.log(
				`${the_bal.title}: `,
				chalk.yellow(`${new_value.toPrecision(9)} ${the_bal.symbol}${change_string}`),
			);
		}
	});
	console.log(chalk.bold.blue(`-------------------`));
...
export const printRewards = (key: string, rewards: number[], old_rewards?: number[]) => {
	...
	rewards.forEach((val, idx) => {
		...
		if (the_reward) {
			...
			console.log(
				`${the_reward.title}: `,
				chalk.yellow(`${new_value.toPrecision(9)} ${the_reward.symbol}${change_string}`),
			);
		}
	});
	console.log(chalk.bold.blue(`-------------------`));
...
export const getTokenPriceFromCoinGecko = async (ticker: string): Promise<number> => {
	...
	const resp: Response = await axios.get(
		`https://pro-api.coingecko.com/api/v3/coins/${ticker}?x_cg_pro_api_key=${process.env.COINGECKO_API_KEY}&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false`,
	);
...
export const rewardRates = async (contract: any, chain: any, reward_tokens: string[], version: number) => {
	try {
...
export const tokenBalanceStakingUnlocked = async (contract: any, wallet_address: string, version: number) => {
	...
	return await contract.methods
		.unlockedBalanceOf(wallet_address)
		.call()
		.then((res) => {
			return contract.methods
				.stakingDecimals()
...
export const tokenBalanceStakingLocked = async (contract: any, wallet_address: string, version: number) => {
	...
	if (version > 2) {
		return await contract.methods
			.lockedLiquidityOf(wallet_address)
			.call()
			.then((res) => {
				return res / 10 ** 18;
			});
		...
		return await contract.methods
			.lockedBalanceOf(wallet_address)
			.call()
			.then((res) => {
				return contract.methods
					.stakingDecimals()
...
export const stakingBalanceBoosted = async (contract: any, wallet_address: string, version: number) => {
	...
	if (version > 2) {
		return await contract.methods
			.combinedWeightOf(wallet_address)
			.call()
			.then((res) => {
				return res / 10 ** 18;
			});
		...
		return await contract.methods
			.boostedBalanceOf(wallet_address)
			.call()
			.then((res) => {
				return contract.methods
					.stakingDecimals()
...
export const tokenBalanceStakingLockedStakes = async (
	...
	if (version >= 1000 && token0_decimals && token1_decimals) {
		...
		stakes = stakes.map((stk, idx) => {
			...
			const the_position = new Position({
				pool: the_pool,
				liquidity: stk.liquidity,
				tickLower: parseInt(stk.tick_lower as any),
				tickUpper: parseInt(stk.tick_upper as any),
			});
